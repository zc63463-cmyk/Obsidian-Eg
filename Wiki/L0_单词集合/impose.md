---
title: "impose"
tags:
  - 学习/英语/词汇
  - 语义场/政治法律
  - 掌握/L0
aliases: []
date: 2026-04-21
mastery: L0
word_freq: 高频
semantic_field: 政治法律
prototype: 放在上面、施加
extension_dim: 空间路径
last_review: 2026-04-21
review_count: 0
---

# impose

> [!info] 基础信息
> **音标** /ɪmˈpəʊz/ | **词频** 考研高频 | **语义场** [[政治法律]]

## 核心释义

**vt.** ①==**强加，推行（税、罚款、限制等）**== ; ②==**把……强加于**==（impose sth. on sb.）

> [!tip] 原型义
> **原型义**：放在上面、施加（物理层面的空间放置）
> **延伸维度**：空间路径
> **隐喻类型**：方位隐喻（上方=权力，"放在上面"=施加压力/限制）

## 词根词缀

im-(=in/on，在……上) + pos(=put/place，放置) + -e（建议创建词根笔记 [[pos/pon]]）
**叙事**：im（在……上）+ pos（放置）→ 把东西"放在"别人头上 → 强加。想象一个人把沉重的包袱"放"在另一个人肩上。

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**放在上面、施加**"为统筹中心，沿空间路径向外扩展：
>
> 1. **放在上面、施加**（物理层面）
>    - → **强加，推行**（vt.）：把规则/税/限制"放在"公众身上
>      - 激活条件：搭配 tax, fine, restriction, ban, sanction 等名词
>    - → **把……强加于**：把想法/意志"放在"别人身上
>      - 激活条件：搭配 impose sth. on/upon sb.

### 统筹

- **一字一词概括**："压"——从上面压下来
- **一句话统筹**：impose 就是从上面"放"下来——无论是政府推行法规，还是个人把意志强加于人。

> [!check]- 链路验证
> - [x] **可逆性**：强加→放在上面→原型义，可逆
> - [x] **可统筹**：所有义项统一于"从上方施加"
> - [x] **有支撑**：impose a tax/ban on 为考研高频搭配

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | vt. | 强加，推行 | 核心词性 |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> im（在……上）+ pose（摆姿势/放置）→ 把东西"摆"在别人身上 → 强加。pose 本身就有"放置"的含义（如 position）。
>
> **词拆分记忆**：
> impose = im（在……上）+ pose（放置）→ 放在别人身上 → 强加。

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[force]] | 强迫 | 用暴力或威胁 | 人/行为/决定 | 强烈贬义 |
> | [[enforce]] | 执行，强制实施 | 强调执行已有规则 | 法律/规则/制裁 | 中性偏强硬 |
> | **impose** | 强加，推行 | 强调自上而下的施加 | 税/限制/观点 | 贬义偏正式 |
>
> **"增"标记**：impose **(增-自上而下)** vs force（impose 更强调"从上面压下来"的权力结构，force 更强调暴力手段）

## 反义词

> [!note]- 反义词 · L2
> - [[remove]]：移除，取消（取消限制 vs 施加限制）
> - [[lift]]：解除，撤销（lift a ban vs impose a ban）

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | imposition | impose + -ition(名词后缀) | 强加，不合理要求 | 词义填充 |
> | imposing | impose + -ing(形容词) | 壮观的，给人深刻印象的 | 词义补充 |

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **impose a tax/ban on**：对……征税/禁令
  > The government imposed a ban on smoking in public places. 政府对公共场所吸烟实施了禁令。
> - **impose sth. on sb.**：把……强加于某人
  > Don't impose your views on others. 不要把你的观点强加于人。

## 真题/语料关联

> [!example]- 语料 · L3
> > - "It can be concluded that girls' attraction to pink seems to be mainly imposed by profit-driven businessmen."（可以得出这样的结论：女孩对粉红色的喜爱似乎主要受到追逐利润的商人的强加。）——2012年考研阅读
> - "Financial regulators in Britain have imposed a rather unusual rule on the bosses of big banks."（英国的金融监管机构对大银行的管理层强加了一条非同寻常的规则。）——2019年考研英语一阅读Text1
> - "Last Thursday, the French Senate passed a digital services tax, which would impose an entirely new tax on large multinationals that provide digital services to consumers or users in France."（上周四，法国参议院通过了一项数字服务税，该税将对向法国消费者或用户提供数字服务的大型跨国公司征收一项全新的税。）——考研阅读真题

%% HERMES-L3: 链接到阅读/翻译笔记中的具体段落 %%

---

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-21 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：
> **翻译实践**：

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：pos/pon（放置）
> - [ ] 同义辨析：impose / force / enforce
> - [ ] 反义词群：remove / lift
> - [ ] 形近词群：impose / impress / import / imply
