---
title: "durable"
tags:
  - 学习/英语/词汇
  - 语义场/自然物理
aliases: []
date: 2026-04-22
word_freq: 必备词
semantic_field: 自然物理
prototype: 持久的，耐用的
extension_dim: [时间路径]
phonetic: "/ˈdjʊərəbl/"
pos: adj.
metaphor_type: 结构隐喻
word_root: dur
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-22
review_count: 0
---

# durable

> [!info] 基础信息
> **音标** /ˈdjʊərəbl/ | **词级** 必备词 | **语义场** [[自然物理]]

## 核心释义

**adj.** ①==**持久的，耐用的**== `durable goods` `durable materials` ; ②持久的（影响、关系等） `durable peace` ;

> [!tip] 原型义
> **原型义**：能够持续很久的
> **延伸维度**：时间路径
> **隐喻类型**：结构隐喻（将"时间长"比作"硬度高"——dur 词根本义就是"坚硬"，硬的东西才能持久）

## 词根词缀

dur(持续，坚硬) + -able(能够...的)
**叙事**：能够(able)持续很久(dur)的——持久的，耐用的。

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**能够持续很久**"为统筹中心，沿时间路径向外扩展：
>
> 1. **能够持续很久**（时间层面）
>    - → **耐用的**：物品能长时间使用而不坏
>      - 激活条件：描述材料、产品的质量
>    - → **持久的**：抽象事物能长时间存在
>      - 激活条件：描述影响、关系、和平等抽象概念

### 统筹（选择适用的模式）

- **延伸中心**：这个词的核心特征是"经得起时间考验"——无论是物品还是抽象事物
- **一句话统筹**：durable 就是"经久不衰"——东西耐用，影响持久

> [!check]- 链路验证
> - [x] **可逆性**：从"持久的（影响）"反推——影响持久就是能在时间长河中持续存在
> - [x] **可统筹**：所有义项统一于"能够持续很久"
> - [x] **有支撑**：真题中 durable peace / durable goods 等搭配常见

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | adj. | 持久的，耐用的 | 基本词性 |
> | n. (durability) | 耐久性，持久性 | adj. → n. 持久的性质 |
> | n. (duration) | 持续时间 | dur + -ation → 持续的状态 |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> 一块坚硬如石(dur)的木板，能够(able)使用很多年——这就是 durable（耐用的）。
>
> **词拆分记忆**（标准词根无法解释时使用）：
> dur（持续）+ -able（能...的）→ 能持续的 → 持久的

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[permanent]] | 永久的 | 强调不会改变 | 变化、状态 | 中性偏正 |
> | [[enduring]] | 持久的 | 强调经受住考验 | 记忆、影响 | 正面 |
> | **durable** | 耐用的，持久的 | 强调经得起时间/使用考验 | 材料、和平、关系 | 中性偏正 |
>
> **"增"标记**：durable **(增-抗损耗感)** vs permanent（后者强调"永远不变"，前者强调"经得起磨损/考验"）

## 反义词

> [!note]- 反义词
> - [[fragile]]：脆弱的，易碎的（与"耐用的"相对）
> - [[temporary]]：暂时的（与"持久的"相对）
> - [[perishable]]：易腐的，不耐久的

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[duration]] | dur + -ation | 持续时间 | 词义补充（持续的长短） |
> | durability | durable + -ity | 耐久性 | 词义填充 |
> | endure | en + dur | 忍受，持续 | 同源词根（dur） |

---

## 搭配与短语

> [!example]- 搭配
> - **durable peace**：The two nations signed a treaty for durable peace.（两国签署了持久和平条约。）
> - **durable goods**：Durable goods sales increased last month.（上月耐用品销量增长。）
> - **durable materials**：The building is made of durable materials.（这座建筑由耐用材料建成。）

## 真题/语料关联

> [!example]- 语料
> - The machines have to be made of durable materials. `[例]`
>   - 中译：这些机器必须由耐用的材料制成。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/durable
> - The trays are made from cedar , which is light and durable. `[例]`
>   - 中译：托盘由雪松制成，轻便耐用。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/durable
> - The resolution calls for a durable peace settlement. `[例]`
>   - 中译：该决议呼吁达成持久和平解决方案。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/durable

---


## 主动产出

> [!success]- 主动产出
> **写作用例**：
> **翻译实践**：
