---
title: "privacy"
tags:
  - 学习/英语/词汇
  - 语义场/政治法律
  - 掌握/L0
aliases: []
date: 2026-04-22
mastery: L0
word_freq: 高频
semantic_field: 政治法律
prototype: 个人的状态
extension_dim: 社会路径
last_review: 2026-04-22
review_count: 0
---

# privacy

> [!info] 基础信息
> **音标** /ˈprɪvəsi/ | **词频** 考研高频 | **语义场** [[政治法律]]

%% HERMES-L0: 填写音标(IPA)、词频(考研高/中/低频)、判定语义场 %%

## 核心释义

**n.** ①==**隐私，私密**== ; ②独处，不受干扰的状态

> [!tip] 原型义
> **原型义**：个人的状态（从公共领域中分离出来，属于个人的）
> **延伸维度**：社会路径
> **隐喻类型**：结构隐喻（隐私=个人领域的"围墙"，将自我与公共空间隔开）

%% HERMES-L0: 判定原型义(物理动作优先>最具体义项优先>最早学到优先>词源义参考) %%

## 词根词缀

priv(个人/单独) + -acy(名词后缀，表状态/性质) = privacy（个人的状态→隐私）

建议创建词根笔记 [[pri]]

**叙事**：每个人都需要一个**个人的**（priv）空间，关上门，不被打扰——这就是**隐私**（-acy）。

%% HERMES-L0: 叙事记忆创作参考哲学文档2.7.4节 %%

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**个人的状态**"为统筹中心，沿社会路径向外扩展：
>
> 1. **个人的状态**（社会层面：从公共领域中分离）
>    - → **隐私，私密**：个人信息不被公开的权利
>      - 激活条件：privacy rights / invade privacy / protect privacy
>    - → **独处，不受干扰**：个人空间不被侵犯
>      - 激活条件：in the privacy of one's home

### 统筹（选择适用的模式）

- **一字一词概括**："个人"——从公共领域中分离出来的个人空间
- **延伸中心**：个人分离→信息不公开→空间不受干扰

> [!check]- 链路验证
> - [x] **可逆性**：独处→个人状态→原型义"个人的状态"，可逆
> - [x] **可统筹**：所有义项围绕"个人/私密"展开
> - [x] **有支撑**：privacy rights / invade privacy 为考研高频用法

%% HERMES-L1: 应用词义链路法操作规程(哲学文档2.2节) %%

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 隐私；独处 | 基础词性 |
> | adj. (private) | 私人的；私密的 | n→adj：隐私的属性 |
> | adv. (privately) | 私下地 | adj→adv：以私人的方式 |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> 在古罗马，**个人的**（priv）事务与公共事务严格分开——关上自家大门的那一刻，**隐私**（privacy）就诞生了。
>
> **词拆分记忆**（标准词根无法解释时使用）：
> priv（= individual，个人）+ acy（名词后缀）→ 个人的状态 → **隐私**

%% HERMES-L1: 优先使用叙事化词源(哲学文档2.7.4) %%

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[secrecy]] | 保密，秘密 | 强调有意隐藏信息 | 政府、组织、行动 | 中性偏贬义 |
> | [[intimacy]] | 亲密，亲密关系 | 强调人际关系的亲近 | 人际关系 | 褒义 |
> | [[privacy]] | ==**隐私**== | 强调个人不受侵扰的权利 | 个人信息、空间、权利 | 中性偏褒义 |
>
> **"增"标记**：privacy **(增-权利属性)** vs secrecy（secrecy 是主动保密，privacy 是个人权利）

%% HERMES-L2: 四维辨析法(哲学文档3.4节) %%

## 反义词

> [!note]- 反义词 · L2
> - [[publicity]]：公开，公众关注（隐私→公开）
> - [[exposure]]：暴露，曝光（私密→暴露）

%% HERMES-L2: 添加wikilink，说明反义关系 %%

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[private]] | priv(个人) + -ate(形容词后缀) | 私人的；私密的 | 词义填充 |
> | [[privately]] | private + -ly(副词后缀) | 私下地 | 词义填充 |
> | [[prior]] | pri(在前面) + -or(比较级) | 先前的；优先的 | pri-族同源 |
> | [[priority]] | prior + -ity(名词后缀) | 优先权 | pri-族同源 |
> | [[privilege]] | priv(个人) + leg(法律) | 特权 | pri-族同源 |
> | [[prudent]] | prud(有远见的) + -ent(形容词后缀) | 谨慎的 | pri-族形义关联 |

%% HERMES-L2: 派生词链接(哲学文档2.5节) %%

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **invade one's privacy**：侵犯某人的隐私
>   The paparazzi were accused of invading the celebrity's privacy.（狗仔队被指控侵犯了这位名人的隐私。）
> - **protect privacy**：保护隐私
>   New regulations have been introduced to protect consumers' online privacy.（新法规出台以保护消费者的在线隐私。）
> - **right to privacy**：隐私权
>   The right to privacy is recognized as a fundamental human right.（隐私权被认定为一项基本人权。）

%% HERMES-L3: 仅列考研真题高频搭配 %%

## 真题/语料关联

> [!example]- 语料 · L3
> - "The protection of online privacy has become one of the most pressing issues in the digital age."（在线隐私保护已成为数字时代最紧迫的问题之一。）——考研高频表达
> - "Many consumers are concerned that their personal data is being collected and used without their knowledge or consent."（许多消费者担心他们的个人数据在未经其知情或同意的情况下被收集和使用。）——考研高频表达
---

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-22 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

%% 复习间隔参考 [[词汇学习哲学#与加工深度等级的对应]] %%

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：
> **翻译实践**：

%% HERMES-L4: 仅在用户实际产出后填写 %%

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：pri（在前面/第一）
> - [ ] 同义辨析：privacy / secrecy / intimacy
> - [ ] 反义词群：publicity / exposure
> - [ ] 形近词群：privacy / private / privilege / privity

%% HERMES-ACTIVATION: AI 根据用户回复勾选对应 checkbox %%
