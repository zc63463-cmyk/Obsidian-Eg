---
title: "pierce"
tags:
  - 学习/英语/词汇
  - 语义场/人体动作
aliases: []
date: 2026-04-21
word_freq: 必备词
semantic_field: 人体动作
prototype: 用尖锐物刺穿
extension_dim: [具身路径]
phonetic: "/pɪəs/"
pos: vt.
metaphor_type: 结构隐喻
word_root: fac
network_activation: [词根, 同义辨析, 反义词群]
last_review: 2026-04-21
review_count: 0
---

# pierce

> [!info] 基础信息
> **音标** /pɪəs/ | **词级** 必备词 | **语义场** [[人体动作]]

## 核心释义

**vt.** ①==**刺穿，穿透**== `pierce sth with sth` / `pierce through` ; ②==**（声音、光线等）穿透，响彻**== `a scream pierces the silence` / `pierce the darkness` ; ③==**打破（沉默、寂静等）**== `pierce the silence/calm` ;
**vi.** ①==**穿入，刺入**== `pierce into/through` ;

> [!tip] 原型义
> **原型义**：用尖锐物刺穿某物
> **延伸维度**：具身路径
> **隐喻类型**：结构隐喻（将"物理刺穿"的结构映射到声音穿透寂静、目光穿透黑暗等抽象场景）

## 词根词缀

pierce ← 古法语 percer ← 拉丁语 pertusus（穿孔的），与 [[fac]](做) 相关（per- + facere = 完全穿透）

%% 涉及已有词根 [[fac]]（做），但 pierce 的词根形态已高度融合，建议创建词根笔记 [[pierce/perc]]（刺穿/穿透） %%

**叙事**：想象一根针刺穿布料——尖锐的力量"穿透"了障碍，这就是 pierce。后来声音、目光、寒风也能"穿透"寂静、黑暗和衣服。

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**用尖锐物刺穿**"为统筹中心，沿具身路径向外扩展：
>
> 1. **用尖锐物刺穿**（物理层面：尖锐物体穿透障碍物）
>    - → **穿透/刺穿**：<保持物理义，扩展到各种穿透场景>
>      - 激活条件：描述针、子弹、箭等穿透物体时
>    - → **声音/光线穿透**：<隐喻化，将物理穿透映射到感官穿透>
>      - 激活条件：描述声音穿透寂静、光线穿透黑暗时
>    - → **打破（沉默等）**：<进一步抽象化，"穿透"沉默就是"打破"沉默>
>      - 激活条件：描述打破沉默、打破平静时

### 统筹（选择适用的模式）

- **延伸中心**：力量"穿透"障碍
- **一句话统筹**：一切"穿透障碍"的动作——针穿透布料，声音穿透寂静，目光穿透黑暗。

> [!check]- 链路验证
> - [x] **可逆性**：从"打破沉默"能反向联想到"刺穿"的原型义
> - [x] **可统筹**：刺穿→穿透→声音穿透→打破沉默，统一于"穿透障碍"
> - [x] **有支撑**：a piercing cry（刺耳的尖叫）/ pierce the silence（打破沉默）

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | vt. | 刺穿；穿透；打破 | 核心及物动词 |
> | vi. | 穿入，刺入 | vt.→vi. 去掉宾语，强调穿入动作 |
> | adj. piercing | 刺骨的；尖锐的 | v.→adj. -ing 表示"正在…的" |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**：
> pierce 的古法语祖先 percer 描绘的是"用尖锐工具穿孔"的动作。想象一个骑士的长矛 pierce（刺穿）了敌人的铠甲——这种"穿透障碍"的力量感，就是 pierce 的核心。
>
> **词拆分记忆**：
> p(形状像矛) + i(我) + erce(发音似"饿死") → 我拿着矛饿死了也要"刺穿"猎物。

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[penetrate]] | 穿透/渗入 | 强调深入内部，过程较缓 | 市场、防线、思想 | 中性 |
> | [[stab]] | 刺 | 强调用刀等利器猛刺 | 人、身体 | 贬义/暴力 |
> | [[prick]] | 扎 | 强调小而浅的刺 | 手指、皮肤 | 中性 |
> | **pierce** | 刺穿 | 强调尖锐物穿透障碍 | 声音、目光、针、矛 | 中性/文学性 |

>
> **"增"标记**：pierce **(增-尖锐性+文学色彩)** vs penetrate（penetrate 更强调深度，pierce 更强调尖锐和力度）

## 反义词

> [!note]- 反义词
> - [[block]]：阻挡（与"穿透"形成对立）
> - [[seal]]：密封（与"刺穿"形成对立）

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | piercing | pierce + -ing | 刺骨的；尖锐的 | 词义补充：形容词化 |

---

## 搭配与短语

> [!example]- 搭配
> - **pierce the silence**：A scream pierced the silence of the night.（一声尖叫打破了夜的寂静。）
> - **pierce through**：The sunlight pierced through the clouds.（阳光穿透了云层。）
> - **piercing cold**：The piercing cold made everyone shiver.（刺骨的寒冷让每个人发抖。）

## 真题/语料关联

> [!example]- 语料
> - The needle pierces the fabric four times a second. `[例]`
>   - 中译：针每秒刺穿织物四次。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/pierce
> - I couldn't wear these earrings because my ears aren't pierced. `[例]`
>   - 中译：我不能戴这些耳环，因为我没有打耳洞。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/pierce
> - The gun fires a shell capable of piercing the armour of an enemy tank. `[例]`
>   - 中译：该枪发射的炮弹能够穿透敌方坦克的装甲。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/pierce

---


## 主动产出

> [!success]- 主动产出
> **写作用例**：
> **翻译实践**：
