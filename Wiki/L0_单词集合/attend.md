---
title: "attend"
tags:
  - 学习/英语/词汇
  - 语义场/行为态度
  - 掌握/L0
aliases: []
date: 2026-04-22
mastery: L0
word_freq: 高频
semantic_field: 行为态度
prototype: 伸展过去（将注意力/身体伸展到某处）
extension_dim: 空间路径
last_review: 2026-04-22
review_count: 0
---

# attend

> [!info] 基础信息
> **音标** /əˈtend/ | **词频** 考研高频 | **语义场** [[行为态度]]

%% HERMES-L0: 填写音标(IPA)、词频(考研高/中/低频)、判定语义场 %%
%% HERMES-L0: 语义场判定规则见 SKILL.md Step 3 %%

## 核心释义

%% HERMES-L0: 按词性分类，仅列考研常考义项(不超过4个)，使用紧凑格式 %%
%% HERMES-L0: 格式: **词性.** ①释义1 ; ②释义2 ; 考研重点义项用 ==**加粗+高亮**== 标记 %%

**v.** ①==**出席，参加**==；②照料，护理；③专心，留意；

> [!tip] 原型义
> **原型义**：伸展过去（将注意力或身体伸展到某处/某人处）
> **延伸维度**：空间路径
> **隐喻类型**：结构隐喻（"伸展过去"→"出席/照料"——身体伸展到现场=出席，注意力伸展到对象=照料）

%% HERMES-L0: 判定原型义(物理动作优先>最具体义项优先>最早学到优先>词源义参考) %%
%% HERMES-L0: 判定主延伸维度(使用频率最高的延伸路径) %%
%% HERMES-L0: 判定隐喻类型(哲学文档2.1节): 方位/结构/本体 %%

## 词根词缀

%% HERMES-L0: 拆分前缀+词根+后缀，检查已有词根笔记并添加[[链接]] %%
%% HERMES-L0: 检查路径: VocabVault/Wiki/词根词缀/ %%

at-(=ad-, 向、朝向) + [[tend-tens|tend]](伸展、倾向)
**叙事**：at（向）+ tend（伸展）→ 将自己伸展到某个地方 → **出席**；将注意力伸展到某人身上 → **照料**。你 attend 一场会议，就是把身体"伸展"到了会场；你 attend 一个病人，就是把关怀"伸展"到了病人身上。

%% HERMES-L0: 叙事记忆创作参考哲学文档2.7.4节 %%
%% HERMES-L0: 如果词根笔记不存在，在输出中提示"建议创建词根笔记 [[<词根>]]" %%

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**伸展过去**"为统筹中心，沿空间路径向外扩展：
>
> 1. **伸展过去**（物理/注意力层面）
>    - → **出席，参加**：将身体伸展到某处（会场、学校）
>      - 激活条件：attend a meeting, attend school
>    - → **照料，护理**：将关怀伸展到某人身上
>      - 激活条件：attend to the patient, attend to one's needs
>    - → **专心，留意**：将注意力伸展到某事上
>      - 激活条件：attend to the problem, attend carefully

### 统筹（选择适用的模式）

%% HERMES-L1: 根据该词的一词多义特征，选择最合适的统筹模式 %%
%% HERMES-L1: 统筹模式参考哲学文档2.3节，以下6种模式选1-2种即可 %%

- **一字一词概括**：attend 就是"伸展过去"——身体伸展到会场=出席，注意力伸展到对象=照料
- **v + 不同宾语**：attend a meeting（出席）→ attend a patient（照料）→ attend to a problem（处理）

> [!check]- 链路验证
> - [x] **可逆性**：从"出席"能反向推导到"身体伸展到会场"
> - [x] **可统筹**：一句话——"attend 就是将自己（身体/注意力）伸展到某处"
> - [x] **有支撑**：attend a meeting, attend to the patient 均有语料支撑

%% HERMES-L1: 应用词义链路法操作规程(哲学文档2.2节) %%
%% HERMES-L1: Step1判定原型义 → Step2确定延伸维度+隐喻类型 → Step3描述路径+激活条件 → Step4选择统筹模式 %%
%% HERMES-L1: 验证标准: 可逆性+可统筹+有支撑 %%

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | v. | 出席；照料；留意 | 核心词性，"伸展过去"的动作 |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

%% HERMES-L1: 分析词性转换路径(哲学文档2.4节) %%
%% HERMES-L1: v→n: 该动作的结果是(什么) / n→n: 子集链路/特性抽象 %%
%% HERMES-L1: a→v: 使动用法 / v→a: +ed表示"持有-动作"的状态 %%

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> at（向）+ tend（伸展）→ 伸展过去。想象一根橡皮筋，你把它从自己身上"伸展"到会场——这就是 attend a meeting；你把它"伸展"到病人身上——这就是 attend to the patient。attend 的核心就是"伸展过去"——身体过去=出席，注意力过去=照料。
>
> **词拆分记忆**（辅助）：
> at（在）+ tend（倾向）→ 倾向于去某处 → 出席

%% HERMES-L1: 优先使用叙事化词源(哲学文档2.7.4)，其次词拆分记忆(2.6节)，再次双编码策略(3.2节) %%
%% HERMES-L1: 词拆分允许创造性拆分(如 contemplate = con + template + plate) %%

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[join]] | 加入，参加 | 强调成为其中一员 | 组织、活动 | 中性 |
> | [[participate]] | 参与 | 强调积极参与并贡献 | 讨论、活动 | 积极 |
> | [[present]] | 出席 | 强调到场状态 | 会议、典礼 | 正式 |
> | **attend** | 出席；照料 | 强调"伸展到现场"的动作感 | 会议、病人 | 中性/通用 |
>
> **"增"标记**：attend **(增-照料义)** vs join/participate（基准词：仅参加）；attend **(增-注意力延伸)** vs present（基准词：仅到场）

%% HERMES-L2: 四维辨析法(哲学文档3.4节) %%
%% HERMES-L2: "增"标记法(哲学文档2.7.1): 标记相对基准词的语义/情感增量 %%
%% HERMES-L2: 检查 Wiki/同义辨析/ 是否已有对应辨析笔记，有则[[链接]] %%

## 反义词

> [!note]- 反义词 · L2
> - [[absent]]：缺席（attend 的精确反义：出席↔缺席）
> - [[ignore]]：忽视（attend 的反义：留意↔忽视）

%% HERMES-L2: 添加[[wikilink]]，说明反义关系 %%

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[attention]] | at-(向) + tent(伸展) + -ion | 注意力 | 词义填充（attend 的名词形式） |
> | [[attitude]] | at-(向) + -titude(倾向) | 态度 | 同前缀族 |
> | [[attempt]] | at-(朝向) + tempt(尝试) | 尝试 | 同前缀族 |
> | [[attribute]] | at-(向) + tribute(给予) | 归因于 | 同前缀族 |
> | [[attendance]] | attend + -ance | 出席率；出席人数 | 词义补充 |

%% HERMES-L2: 派生词链接(哲学文档2.5节) %%
%% HERMES-L2: 词义补充: 派生词反哺母词 / 词义填充: 母词填充派生词 %%

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **attend a meeting/conference**：参加会议/会议
> - **attend school**：上学
> - **attend to**：处理；照料
> - **attend on**：伺候，照料

%% HERMES-L3: 仅列考研真题高频搭配，不编造 %%
%% HERMES-L3: 每个搭配附带1个真题例句 %%

## 真题/语料关联

> [!example]- 语料 · L3
> - "I have more pressing things to attend to."（我有更紧迫的事情要处理。——考研英语高频短语）
> - "The first step, as Erik Brynjolfsson and Andrew McAfee argue in The Second Machine Age, should be rethinking education and job training."（正如Erik Brynjolfsson和Andrew McAfee在《第二次机器时代》中所主张的，第一步应该是重新思考教育和职业培训。——2018年考研英语阅读，attend 的语境关联：attend school → education）

%% HERMES-L3: 链接到阅读/翻译笔记中的具体段落 %%

---

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-22 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

%% 复习间隔参考 [[词汇学习哲学#与加工深度等级的对应]] %%

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>

%% HERMES-L4: 仅在用户实际产出后填写 %%

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：[[tend-tens|tend-/tens-/tent-]]（伸展、倾向，拉丁语 *tendere*）
> - [ ] 同义辨析：attend/join/participate/present
> - [ ] 反义词群：absent/ignore
> - [ ] 形近词群：attend/attention/attitude/attempt/attribute/intend/extend/contend

%% HERMES-ACTIVATION: AI 根据用户回复勾选对应 checkbox，/回顾时读取勾选状态决定是否纳入对应网络 %%
