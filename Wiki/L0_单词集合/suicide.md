---
title: "suicide"
tags:
  - 学习/英语/词汇
  - 语义场/身心状态
  - 掌握/L0
aliases: []
date: 2026-04-22
mastery: L0
word_freq: 中频
semantic_field: 身心状态
prototype: 自己杀死自己
extension_dim: 具身路径
last_review: 2026-04-22
review_count: 0
---

# suicide

> [!info] 基础信息
> **音标** /ˈsuːɪsaɪd/ | **词频** 考研中频 | **语义场** [[身心状态]]

## 核心释义

**n.** ①==**自杀**== ; ②自取灭亡的行为 ;
**v.** ①自杀 ;

> [!tip] 原型义
> **原型义**：自己杀死自己（sui- 自己 + cid 杀）
> **延伸维度**：具身路径
> **隐喻类型**：无隐喻

## 词根词缀

sui-（自己，拉丁语 *suus*）+ cid（切/杀，建议创建词根笔记 [[cid]]）
**叙事**：一个人拿起刀**切**向**自己**——这就是 suicide（自杀）。sui（自己）+ cid（切/杀）= 自己杀自己 = suicide。

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**自己杀死自己**"为统筹中心，沿具身路径向外扩展：
>
> 1. **自己杀死自己**（物理层面：夺去自己的生命）
>    - → ==**自杀**==：主动结束自己的生命
>      - 激活条件：commit suicide（自杀）；搭配 rate, attempt, prevention
>    - → **自取灭亡的行为**：比喻性的自我毁灭
>      - 激活条件：政治/商业语境，如 political suicide

### 统筹（选择适用的模式）

- **一字一词概括**：自毁——自己毁灭自己
- **一句话统筹**：suicide 就是"自己杀自己"，字面是自杀，引申为一切自取灭亡的行为。

> [!check]- 链路验证
> - [x] **可逆性**：从"自杀"可反推回"自己杀自己"
> - [x] **可统筹**：一句话"自己杀自己"串联所有义项
> - [x] **有支撑**：考研常见搭配

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 自杀；自取灭亡 | 原型义 |
> | v. | 自杀 | n. → v.：名词动用 |

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> sui（自己）+ cid（切/杀）。一个人拿起刀**切**向**自己**——这是最直白的词源记忆。记住：自己杀 = suicide。

> **词拆分记忆**（辅助）：
> sui（发音似"碎"）+ cid（切）→ 把自己切碎 → suicide（自杀）

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[suicide]] | 自杀（字面义） | 直接、明确 | 人 | 消极 |
> | [[self-destruction]] | 自我毁灭（比喻义） | 抽象、比喻 | 行为/政策 | 消极 |
>
> **"增"标记**：suicide **(增-字面直接性)** vs self-destruction

## 反义词

> [!note]- 反义词 · L2
> - [[survive]]：幸存（活下去）
> - [[rescue]]：拯救（救回生命）

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | suicidal | suicide + -al（形容词） | 有自杀倾向的；自毁的 | 词义填充 |

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **commit suicide**：自杀
  - He committed suicide last year.（他去年自杀了。）
> - **suicide rate**：自杀率
  - The suicide rate has been declining.（自杀率一直在下降。）

## 真题/语料关联

> [!example]- 语料 · L3
> - "The Supreme Court's decisions on physician-assisted suicide carry important implications for how medicine seeks to relieve dying patients of pain and suffering."（最高法院关于医生协助自杀的裁决，对于医学界寻求如何减轻垂死病人痛苦具有重要意义——2002年考研Text 4）
> - "On another level, many in the medical community acknowledge that the assisted-suicide debate has been fueled in part by the despair of patients for whom modern medicine has prolonged the physical agony of dying."（在另一层面，很多医学界的人承认，协助自杀争议部分是由于病人极度绝望引起的，对他们而言，现代医学延长了他们死亡前的肉体痛苦——2002年考研Text 4）

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-22 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：待填写
> **翻译实践**：待填写

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：cid/cise（切/杀）——建议创建词根笔记 [[cid]]
> - [ ] 同义辨析：suicide / self-destruction
> - [ ] 反义词群：survive / rescue
> - [ ] 形近词群：suicide / suit / suitable / sue
