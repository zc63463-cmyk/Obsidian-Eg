---
title: "substantial"
tags:
  - 学习/英语/词汇
  - 语义场/理性世界
  - 掌握/L0
aliases: []
date: 2026-04-22
mastery: L0
word_freq: 高频
semantic_field: 理性世界
prototype: 具有大量根基的
extension_dim: 抽象路径
last_review: 2026-04-22
review_count: 0
---

# substantial

> [!info] 基础信息
> **音标** /səbˈstænʃəl/ | **词频** 考研高频 | **语义场** [[理性世界]]

%% HERMES-L0: 填写音标(IPA)、词频(考研高/中/低频)、判定语义场 %%
%% HERMES-L0: 语义场判定规则见 SKILL.md Step 3 %%

## 核心释义

%% HERMES-L0: 按词性分类，仅列考研常考义项(不超过4个)，使用紧凑格式 %%
%% HERMES-L0: 格式: **词性.** ①释义1 ; ②释义2 ; 考研重点义项用 ==**加粗+高亮**== 标记 %%

**adj.** ①==**大量的，相当可观的**== ; ②==**实质的，有重大价值的**== ; ③坚固的，结实的

> [!tip] 原型义
> **原型义**：具有大量根基的（有实质支撑的）
> **延伸维度**：抽象路径
> **隐喻类型**：结构隐喻（"根基厚实"→"数量大/价值高"）

%% HERMES-L0: 判定原型义(物理动作优先>最具体义项优先>最早学到优先>词源义参考) %%
%% HERMES-L0: 判定主延伸维度(使用频率最高的延伸路径) %%
%% HERMES-L0: 判定隐喻类型(哲学文档2.1节): 方位/结构/本体 %%

## 词根词缀

%% HERMES-L0: 拆分前缀+词根+后缀，检查已有词根笔记并添加[[链接]] %%
%% HERMES-L0: 检查路径: VocabVault/Wiki/词根词缀/ %%

sub-(在下面) + 建议创建词根笔记 [[st/sta]](站立) + -ant(名词/形容词后缀) + -ial(形容词后缀) = 根基厚实的
**叙事**：一棵大树根基（st站立）深埋地下（sub-在下），根基越厚实，树冠就越庞大——substantial 就是"根基厚实的→大量的"。

%% HERMES-L0: 叙事记忆创作参考哲学文档2.7.4节 %%
%% HERMES-L0: 如果词根笔记不存在，在输出中提示"建议创建词根笔记 [[<词根>]]" %%

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**根基厚实的**"为统筹中心，沿抽象路径向外扩展：
>
> 1. **根基厚实的**（物理层面：底座大而结实）
>    - → **大量的，相当可观的**：根基厚实→体量大→数量多
>      - 激活条件：修饰数量、金额、证据等可量化名词
>    - → **实质的，有重大价值的**：根基厚实→有真正支撑→非表面的
>      - 激活条件：修饰变化、贡献、差异、论点等抽象名词
>    - → **坚固的，结实的**：根基厚实→物理上结实
>      - 激活条件：修饰建筑、家具等物理实体

### 统筹（选择适用的模式）

%% HERMES-L1: 根据该词的一词多义特征，选择最合适的统筹模式 %%
%% HERMES-L1: 统筹模式参考哲学文档2.3节，以下6种模式选1-2种即可 %%

- **一字一词概括**："厚实"——根基厚实所以量大，根基厚实所以有实质
- **一句话统筹**：substantial 的核心是"根基厚实"——根基厚则量大，根基厚则有实质

> [!check]- 链路验证
> - [x] **可逆性**：从引申义能否反向推导回原型义？
> - [x] **可统筹**：能否用一句话将所有义项串联？
> - [x] **有支撑**：是否有真题例句或语料支撑该链路？

%% HERMES-L1: 应用词义链路法操作规程(哲学文档2.2节) %%
%% HERMES-L1: Step1判定原型义 → Step2确定延伸维度+隐喻类型 → Step3描述路径+激活条件 → Step4选择统筹模式 %%
%% HERMES-L1: 验证标准: 可逆性+可统筹+有支撑 %%

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | adj. | 大量的；实质的 | 基本词性 |
> | n. (substance) | 物质；实质 | adj → n：去掉 -ial，核心名词形式 |
> | adv. (substantially) | 大量地；实质上 | adj + -ly → 副词化 |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

%% HERMES-L1: 分析词性转换路径(哲学文档2.4节) %%
%% HERMES-L1: v→n: 该动作的结果是(什么) / n→n: 子集链路/特性抽象 %%
%% HERMES-L1: a→v: 使动用法 / v→a: +ed表示"持有-动作"的状态 %%

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> 大树的根基（st站立）深埋地下（sub-在下），根基越厚实，树冠就越庞大——substantial 就是"根基厚实的→大量的/实质的"。
>
> **词拆分记忆**（标准词根无法解释时使用）：
> sub（下面）+ stance（站）+ ial → 站在下面的根基很厚实 → 大量的/实质的
>
> **其他**：联想 [[substance]]（实质）→ substantial（有实质的/大量的）

%% HERMES-L1: 优先使用叙事化词源(哲学文档2.7.4)，其次词拆分记忆(2.6节)，再次双编码策略(3.2节) %%
%% HERMES-L1: 词拆分允许创造性拆分(如 contemplate = con + template + plate) %%

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[considerable]] | 相当多的 | 侧重值得考虑的数量 | 数量、时间、精力 | 中性 |
> | [[significant]] | 重要的，显著的 | 侧重意义和影响 | 变化、差异、贡献 | 积极偏正式 |
> | **substantial** | ==**根基厚实的→量大且有实质**== | 侧重有根基支撑的"多" | 金额、证据、贡献 | 中性偏积极 |
>
> **"增"标记**：substantial **(增-根基支撑语义)** vs considerable（substantial 强调"有根基"的多，considerable 侧重数量本身多）

%% HERMES-L2: 四维辨析法(哲学文档3.4节) %%
%% HERMES-L2: "增"标记法(哲学文档2.7.1): 标记相对基准词的语义/情感增量 %%
%% HERMES-L2: 检查 Wiki/同义辨析/ 是否已有对应辨析笔记，有则[[链接]] %%

## 反义词

> [!note]- 反义词 · L2
> - [[insubstantial]]：无实质的，不牢固的
> - [[nominal]]：名义上的（有substantial的反义意味）

%% HERMES-L2: 添加[[wikilink]]，说明反义关系 %%

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[substantially]] | substantial + -ly | 大量地；实质上 | 词义补充 |
> | [[substance]] | sub + st(ant) + -ance | 物质；实质 | 词义填充 |
> | [[substantive]] | sub + st(ant) + -ive | 实质性的 | 词义补充 |
> | [[submit]] | sub + mit | 提交；屈服 | 同族互链 |
> | [[subordinate]] | sub + ordin + -ate | 下级的 | 同族互链 |
> | [[subscribe]] | sub + scrib(e) | 订阅；签署 | 同族互链 |
> | [[subsequent]] | sub + sequ + -ent | 随后的 | 同族互链 |
> | [[substitute]] | sub + stitut(e) | 替代品 | 同族互链 |
> | [[subtle]] | sub + tl(e) | 微妙的 | 同族互链 |

%% HERMES-L2: 派生词链接(哲学文档2.5节) %%
%% HERMES-L2: 词义补充: 派生词反哺母词 / 词义填充: 母词填充派生词 %%

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **a substantial amount/sum of**：大量的……
  - A substantial amount of money was raised for the charity.（为慈善机构筹集了大量资金。）
> - **substantial evidence**：实质性的证据
  - The police found substantial evidence linking him to the crime.（警方找到了将他与犯罪联系起来的实质性证据。）
> - **substantial difference**：重大差异
  - There is a substantial difference between the two proposals.（两个提案之间存在重大差异。）

%% HERMES-L3: 仅列考研真题高频搭配，不编造 %%
%% HERMES-L3: 每个搭配附带1个真题例句 %%

## 真题/语料关联

> [!example]- 语料 · L3
> - "Indeed, we have made substantial progress in some parts of the world on at least one of these — the distribution of family-planning services."（事实上，在世界上某些地区，我们至少在其中一方面——计划生育服务的分配——取得了实质性进展。）——考研阅读真题
> - "The house is built of substantial stone."（这房子是用坚固的石头建造的。）——2012年考研英语一Text1
> - "She sold the restaurant with a substantial profit."（她以可观的利润卖掉了那家餐厅。）——考研高频表达

---

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-22 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

%% 复习间隔参考 [[词汇学习哲学#与加工深度等级的对应]] %%

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>

%% HERMES-L4: 仅在用户实际产出后填写 %%

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：sub（在下面）+ st/sta（站立）
> - [ ] 同义辨析：substantial / considerable / significant
> - [ ] 反义词群：insubstantial / nominal
> - [ ] 形近词群：substantial / substance / substantive

%% HERMES-ACTIVATION: AI 根据用户回复勾选对应 checkbox，/回顾时读取勾选状态决定是否纳入对应网络 %%
