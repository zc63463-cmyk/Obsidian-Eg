---
title: "resent"
tags:
  - 学习/英语/词汇
  - 语义场/人本世界
aliases: []
date: 2026-04-22
word_freq: 必备词
semantic_field: 人本世界
prototype: 反复感到（痛苦/愤怒）
extension_dim: [抽象路径]
phonetic: "/rɪˈzent/"
pos: vt.
metaphor_type: 本体隐喻
word_root: sens-sent
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-22
review_count: 0
---

# resent

> [!info] 基础信息
> **音标** /rɪˈzent/ | **词级** 必备词 | **语义场** [[人本世界]]

## 核心释义

**vt.** ①==**怨恨，愤恨==`resent doing sth.` / `resent sb. for sth.` ;

> [!tip] 原型义
> **原型义**：反复感到（痛苦、愤怒），在心中反复咀嚼不满
> **延伸维度**：抽象路径
> **隐喻类型**：本体隐喻（情感=反复咀嚼的苦果）

## 词根词缀

re-(反复) + [[sent|sent]](感觉) → 反复感到不满 → 怨恨
**叙事**：被不公平对待后，心中反复涌起愤怒的感觉，这就是 resent。

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**反复感到不满**"为统筹中心，沿抽象路径向外扩展：
>
> 1. **反复感到不满**（情感层面：对某事持续感到愤怒）
>    - → **怨恨、愤恨**：因受到不公正对待而产生的持续愤怒
>      - 激活条件：后接名词或动名词（resent doing sth.），表达对某行为的不满

### 统筹（选择适用的模式）

- **一字一词概括**："恨"——反复在心中感到的不满
- **一句话统筹**：resent 就是把不满反复在心里翻来覆去地感受，最终凝结为怨恨。

> [!check]- 链路验证
> - [x] **可逆性**：从"怨恨"可反向推导至"反复感到不满"
> - [x] **可统筹**：一个"恨"字概括核心
> - [x] **有支撑**：resent being treated unfairly 为典型用法

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | vt. | 怨恨，愤恨 | 核心动词 |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> re-(反复) + sent(感觉) → 被冒犯后反复感到愤怒 = 怨恨。
>
> **词拆分记忆**（标准词根无法解释时使用）：
> re(反复) + sent(送出) → 反复送出愤怒的情绪 = 怨恨。

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[hate]] | 仇恨，憎恨 | 最强烈，情感最直接 | 人、事物 | 贬义/强烈 |
> | [[dislike]] | 不喜欢 | 较温和，日常用语 | 人、事物 | 中性偏贬 |
> | **resent** | 怨恨，愤恨 | 强调"因不公而产生的持续不满" | 对待方式、行为 | 贬义/隐忍 |
>
> **"增"标记**：resent **(增-不公正感和隐忍性)** vs hate

## 反义词

> [!note]- 反义词
> - [[appreciate]]：感激（与 resent 的"怨恨"形成反义）
> - [[forgive]]：原谅（与 resent 的"记恨"形成反义）

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | resentment | resent + -ment(名词后缀) | 怨恨，愤恨 | 词义补充 |
> | resentful | resent + -ful(充满) | 充满怨恨的 | 词义填充 |
> | [[resent]] | — | 怨恨 | 母词 |

---

## 搭配与短语

> [!example]- 搭配
> - **resent doing sth.**：怨恨做某事
> - **resent sb. for sth.**：因某事怨恨某人
> - **deeply resent**：深深地怨恨

## 真题/语料关联

> [!example]- 语料
> - `[真题]` Indeed, the mere presence of a grape in the other chamber was enough to induce resentment in a female capuchin.
> - `[COCA-ACAD]` He deeply resented the fact that his teammates did not support him.
> - `[COCA-ACAD]` Many employees resent being excluded from the decision-making process.

---


## 主动产出

> [!success]- 主动产出
> **写作用例**：
> **翻译实践**：
