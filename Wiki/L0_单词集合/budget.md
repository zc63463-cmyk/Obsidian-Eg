---
title: "budget"
tags:
  - 学习/英语/词汇
  - 语义场/资本经济
aliases: []
date: 2026-04-22
word_freq: 必备词
semantic_field: 资本经济
prototype: 皮革钱袋、预算
extension_dim: [抽象路径]
phonetic: "/ˈbʌdʒɪt/"
pos: n.
metaphor_type: 本体隐喻
word_root: budg
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-22
review_count: 0
---

# budget

> [!info] 基础信息
> **音标** /ˈbʌdʒɪt/ | **词级** 必备词 | **语义场** [[资本经济]]

## 核心释义

**n.** ①==**预算**== `N` [一般]；②（有限的）经费 `a N` [一般]；
**v.** ①==**编预算，规划开支**== `V for N` [一般]；②谨慎花钱 `V` [一般]；
**adj.** ①廉价的，经济型的 `adj N` [商业]；
> [!tip] 原型义
> **原型义**：皮革钱袋（装钱的袋子）→ 预算
> **延伸维度**：抽象路径
> **隐喻类型**：本体隐喻——从"装钱的袋子"到"对钱的规划"

## 词根词缀

budg-(来自古法语 bouge，皮革袋子、钱包) + -et(指小后缀) → 小钱包 → 预算
**叙事**：古代财务官把计划要花的钱装进一个小**皮革袋**（bouget）里，袋子里有多少钱就是**预算**（budget）有多少。后来 budget 从"钱袋"变成了"对钱的规划"。与 [[build]] 同源——都来自"建造/构建"的概念族。

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**装钱的袋子 → 预算**"为统筹中心，沿抽象路径向外扩展：
>
> 1. **装钱的袋子**（物理层面：钱袋）
>    - → **预算，经费**：<钱袋里装的钱 = 计划开支>
>      - 激活条件：搭配 annual, government, tight，财务语境
>    - → **编预算，安排开支**：<做预算的动作>
>      - 激活条件：搭配 for，表示为某事做预算
>    - → **廉价的，经济型的**：<在预算内的 = 省钱的>
>      - 激活条件：搭配 airline, hotel，消费语境

### 统筹（选择适用的模式）

- **延伸中心**："钱袋"→ 袋中的钱（预算）→ 管理袋中的钱（编预算）→ 袋中钱不多（廉价）
- **一字一词概括**："算"——预算、算计、划算

> [!check]- 链路验证
> - [x] **可逆性**：从"廉价的"可反向推导——在预算内 → 预算 → 钱袋
> - [x] **可统筹**：所有义项都围绕"对财务的规划/限制"
> - [x] **有支撑**：tight budget / budget airline 为考研高频搭配

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 预算；经费 | 原型义：钱袋 → 预算 |
> | v. | 编预算；谨慎花钱 | n→v：做预算的动作 |
> | adj. | 廉价的，经济型的 | n→adj：在预算内的 → 省钱的 |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**：
> 想象一个古代商人打开他的**皮革钱袋**（bouget），数了数里面的金币——"这就是我这个月的**预算**（budget）"。如果钱袋很瘪，他就只能住**廉价**的（budget）旅馆。
>
> **词拆分记忆**：
> budget = budg(皮革袋) + -et(小) → 小钱袋 → 预算

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[allowance]] | 津贴，零花钱 | 偏个人/家庭 | 个人开支 | 中性 |
> | [[fund]] | 基金，资金 | 偏机构/项目 | 项目、研究 | 中性 |
> | [[estimate]] | 估算 | 强调预估过程 | 成本、费用 | 中性 |
> | **budget** | 预算；经费 | 强调计划性 | 政府、企业、个人 | 中性 |

## 反义词

> [!note]- 反义词
> - [[luxury]]：budget（廉价的）vs luxury（奢侈的）
> - [[extravagance]]：budget（预算）vs extravagance（挥霍）

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[build]] | 同源概念族 | 建造，构建 | 词义补充 |
> | budgetary | budget + -ary | 预算的 | 词义填充 |

---

## 搭配与短语

> [!example]- 搭配
> - **tight budget**：紧张的预算（We have to complete the project on a tight budget. 我们必须在紧张的预算内完成这个项目。）
> - **within/beyond budget**：在预算内/超出预算（The renovation was completed within budget. 翻修在预算内完成了。）
> - **budget deficit**：预算赤字（The government is trying to reduce the budget deficit. 政府正试图减少预算赤字。）

## 真题/语料关联

> [!example]- 语料
> - The firm has drawn up a budget for the coming financial year. `[例]`
>   - 中译：该公司已经制定了下一个财政年度的预算。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/budget
> - Libraries are finding it increasingly difficult to remain within budget. `[例]`
>   - 中译：图书馆发现越来越难以保持预算之内。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/budget
> - The app sets a budget for your calorie intake. `[例]`
>   - 中译：该应用程序为您的卡路里摄入量设定预算。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/budget

---


## 主动产出

> [!success]- 主动产出
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>
