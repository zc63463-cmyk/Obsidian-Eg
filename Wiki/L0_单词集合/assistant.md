---
title: "assistant"
tags:
  - 学习/英语/词汇
  - 语义场/社会习俗
aliases: []
date: 2026-04-22
word_freq: 必备词
semantic_field: 社会习俗
prototype: 站在旁边帮助的人
extension_dim: [社会路径]
phonetic: "/əˈsɪstənt/"
pos: n.
metaphor_type: 方位隐喻
word_root: st-sta-stat
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-22
review_count: 0
---

# assistant

> [!info] 基础信息
> **音标** /əˈsɪstənt/ | **词级** 必备词 | **语义场** [[社会习俗]]

## 核心释义

**n.** ①==**助手，助理**== `N to N` ; ②助教 `N`
**adj.** ①==**助理的，副的**== `adj N`

> [!tip] 原型义
> **原型义**：站在旁边帮助的人
> **延伸维度**：社会路径
> **隐喻类型**：方位隐喻（从物理上的"站在旁边"→社会角色中的"助手"）

## 词根词缀

as-(=ad-，朝向) + [[st-sta-stat|sist]](=st-，站立) + -ant(名词后缀，表示人)
**叙事**：站在某人旁边提供帮助的**人**——就是"助手"。

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**站在旁边帮助的人**"为统筹中心，沿社会路径向外扩展：
>
> 1. **站在旁边帮助的人**（社会层面：提供辅助的人）
>    - → **助手、助理**（职业层面：正式的辅助工作人员）
>      - 激活条件：描述职场、学术中的辅助人员
>    - → **助理的、副的**（修饰层面：表示辅助性质的）
>      - 激活条件：描述职位头衔中的修饰语

### 统筹（选择适用的模式）

- **延伸中心**：以"在旁提供支持的人"为特征，从帮助者延伸到职业角色
- **一句话统筹**：所有义项都围绕"提供辅助支持的人或属性"

> [!check]- 链路验证
> - [x] **可逆性**：从"research assistant"可反向推导到"站在旁边帮助的人"（站在研究者旁边协助的人）
> - [x] **可统筹**：一句话"提供辅助支持的人或属性"串联所有义项
> - [x] **有支撑**：考研中"teaching/research assistant"为高频搭配

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 助手、助理 | 基本词性 |
> | adj. | 助理的、副的 | n→adj：零派生，"助手"→"具有助手性质的" |
> | v. ([[assist]]) | 帮助、协助 | n→v：-ant→零派生，"助手"→"执行帮助的动作" |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> [[assist]]（站在旁边帮助）+ -ant（人）→ 站在旁边帮助的**人**——就是"助手"。-ant 后缀专门标记"做某事的人"，像 student（学习的人）、assistant（帮助的人）。
>
> **词拆分记忆**：
> assist（协助）+ ant（蚂蚁）→ 像蚂蚁一样勤勤恳恳站在旁边帮忙的人——"助手"。

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[helper]] | 帮手 | 口语化，通用 | 日常事务 | 正面/随意 |
> | [[aide]] | 助手 | 强调在高级别人身边辅助 | 政治家、军官 | 正面/正式 |
> | **assistant** | 助手、助理 | 强调正式的辅助职位 | 学术、商业、行政 | 正面/正式 |
>
> **"增"标记**：assistant **(增-职业性/正式性)** vs helper

## 反义词

> [!note]- 反义词
> - [[supervisor]]：主管（助手的反面——管理者）
> - [[boss]]：老板（辅助者的反面——主导者）

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[assist]] | assistant 去掉 -ant | 帮助、协助 | 词义补充：动词形式补充动作义 |
> | [[assistance]] | assist + -ance(名词后缀) | 帮助、援助 | 词义补充：名词形式补充抽象义 |
> | assistantship | assistant + -ship(身份) | 助教奖学金 | 专业术语 |

---

## 搭配与短语

> [!example]- 搭配
> - **teaching assistant (TA)**：助教（She works as a teaching assistant in the physics department.）
> - **research assistant**：研究助理（He was hired as a research assistant for the project.）
> - **assistant professor**：助理教授（She was promoted to assistant professor.）

## 真题/语料关联

> [!example]- 语料
> - She's a personal assistant as distinct from a secretary. `[例]`
>   - 中译：她是一名私人助理，与秘书不同。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/assistant
> - She can't accept she made a mistake and now she's trying to lay the blame on her assistant. `[例]`
>   - 中译：她无法接受自己犯了一个错误，现在她试图将责任归咎于她的助理。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/assistant
> - Did you want to talk to the chairman himself, or could his personal assistant help you? `[例]`
>   - 中译：您想与主席本人交谈吗？或者他的私人助理可以帮助您吗？
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/assistant

---


## 主动产出

> [!success]- 主动产出
> **写作用例**：
> **翻译实践**：
