---
title: "mixture"
tags:
  - 学习/英语/词汇
  - 语义场/抽象关系
aliases: []
date: 2026-04-22
word_freq: 必备词
semantic_field: 抽象关系
prototype: 混合物，混合
extension_dim: [具身路径]
phonetic: "/ˈmɪkstʃər/"
pos: n.
metaphor_type: 结构隐喻
word_root: mix
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-22
review_count: 0
---

# mixture

> [!info] 基础信息
> **音标** /ˈmɪkstʃər/ | **词级** 必备词 | **语义场** [[抽象关系]]

## 核心释义

**n.** ①==**混合物**== `N` `adj N`；②==**混合，混合状态**== `N`；

> [!tip] 原型义
> **原型义**：混合物——由两种或多种不同物质混合而成的东西
> **延伸维度**：具身路径
> **隐喻类型**：结构隐喻（混合物=多种元素整合的结构）

## 词根词缀

mix(混合) + -ture(名词后缀，表示动作的结果或状态)
**叙事**：把咖啡和牛奶 **mix**（混合）在一起，得到一杯 **mixture**（混合物）——拿铁。

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**混合物**"为统筹中心，沿具身路径向外扩展：
>
> 1. **混合物**（物理层面：多种物质混合的产物）
>    - → **混合状态**：从具体产物抽象为混合的状态
>      - 激活条件：描述事物处于混合状态时（a mixture of...）

### 统筹（选择适用的模式）

- **延伸中心**：以"多种元素的融合"为中心，从具体产物到抽象状态
- **一句话统筹**：mixture 就是 mix 的结果——多种元素融合的产物或状态

> [!check]- 链路验证
> - [x] **可逆性**：从"混合状态"可反推回"混合物"
> - [x] **可统筹**：所有义项围绕"混合的结果"串联
> - [x] **有支撑**：a mixture of / chemical mixture 等常见用法

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 混合物 | 基础词性，[[mix]] 的名词形式 |
> | v. | 混合 | n.→v. [[mix]]，动作本身 |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> 想象一个化学实验：你把蓝色液体和黄色液体倒进同一个烧杯，搅拌后变成了绿色——这个绿色的 **mixture**（混合物）是两种原色的融合。

> **词拆分记忆**：
> [[mix]](混合) + ture(像"ture"→ture 真实) → 混合后变成真实的产物 → **混合物**

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[compound]] | 化合物 | 强调化学层面的化合 | 化学物质 | 中性/科学 |
> | [[blend]] | 混合物 | 强调和谐融合 | 饮料、香料、文化 | 正面 |
> | **mixture** | 混合物 | 强调多种成分的物理混合 | 材料、物质、人群 | 中性 |
>
> **"增"标记**：mixture **(增-通用性)** vs compound，mixture 更通用，不限于化学语境

## 反义词

> [!note]- 反义词
> - [[pure substance]]：纯净物——与混合物相对
> - [[element]]：元素——单一成分

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[mix]] | 基础词 | 混合 | 词义填充：母词填充派生词 |
> | mixed | mix + -ed | 混合的 | 词义补充：形容词形式 |
> | mixer | mix + -er | 搅拌器 | 词义补充：工具名词 |

---

## 搭配与短语

> [!example]- 搭配
> - **a mixture of**：The city is a fascinating mixture of old traditions and modern technology.
> - **chemical mixture**：The laboratory produced a new chemical mixture with unique properties.

## 真题/语料关联

> [!example]- 语料
> - Throughout the trip , Jackson juggled multiple cameras and plate sizes using the " collodion process" that required him to coat the plates with a chemical mixture , then expose them and develop the resulting images with a portable darkroom. `[真题]`
>   - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/mixture
>   - 中译：【需人工审核】与 mixture 相关的例句。
> - The mixture of flour , water , and yeast is then left in a warm place for four hours. `[例]`
>   - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/mixture
>   - 中译：【需人工审核】与 mixture 相关的例句。
> - Their latest CD is a mixture of new and old songs. `[例]`
>   - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/mixture
>   - 中译：【需人工审核】与 mixture 相关的例句。

---
>   - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/mixture


## 主动产出

> [!success]- 主动产出
> **写作用例**：
> **翻译实践**：
