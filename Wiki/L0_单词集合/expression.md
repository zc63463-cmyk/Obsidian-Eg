---
title: "expression"
tags:
  - 学习/英语/词汇
  - 语义场/言语表达
  - 掌握/L0
aliases: []
date: 2026-04-22
mastery: L0
word_freq: 高频
semantic_field: 言语表达
prototype: 挤压出来的结果（表达的过程或产物）
extension_dim: 抽象路径
last_review: 2026-04-22
review_count: 0
---

# expression

> [!info] 基础信息
> **音标** /ɪkˈspreʃn/ | **词频** 考研高频 | **语义场** [[言语表达]]

%% HERMES-L0: 填写音标(IPA)、词频(考研高/中/低频)、判定语义场 %%
%% HERMES-L0: 语义场判定规则见 SKILL.md Step 3 %%

## 核心释义

%% HERMES-L0: 按词性分类，仅列考研常考义项(不超过4个)，使用紧凑格式 %%
%% HERMES-L0: 格式: **词性.** ①释义1 ; ②释义2 ; 考研重点义项用 ==**加粗+高亮**== 标记 %%

**n.** ①==**表达，表述**==；②==**表情，神色**==；③措辞，表达方式；④（艺术）表现力；

> [!tip] 原型义
> **原型义**：挤压出来的结果（把内在东西向外压出来的过程或产物）
> **延伸维度**：抽象路径
> **隐喻类型**：结构隐喻（内心的想法被比作被挤压的物质——挤压后的产物就是 expression）

%% HERMES-L0: 判定原型义(物理动作优先>最具体义项优先>最早学到优先>词源义参考) %%
%% HERMES-L0: 判定主延伸维度(使用频率最高的延伸路径) %%
%% HERMES-L0: 判定隐喻类型(哲学文档2.1节): 方位/结构/本体 %%

## 词根词缀

%% HERMES-L0: 拆分前缀+词根+后缀，检查已有词根笔记并添加[[链接]] %%
%% HERMES-L0: 检查路径: VocabVault/Wiki/词根词缀/ %%

ex-(=out, 向外) + press(=压，挤压) + -ion(名词后缀，表示动作或结果)
**叙事**：ex（向外）+ press（压）+ -ion（结果）→ **向外压出来的结果** → 表达。[[express]] 是"压出来"的动作，[[expression]] 是压出来的结果——你的表情就是内心情感"压出来"后留在脸上的痕迹。

%% HERMES-L0: 叙事记忆创作参考哲学文档2.7.4节 %%
%% HERMES-L0: 如果词根笔记不存在，在输出中提示"建议创建词根笔记 [[<词根>]]" %%
建议创建词根笔记 [[press]]（压、挤压，拉丁语 *premere*）

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**挤压出来的结果**"为统筹中心，沿抽象路径向外扩展：
>
> 1. **挤压出来的结果**（产物层面）
>    - → **表达，表述**：把想法压出来变成语言的结果
>      - 激活条件：freedom of expression, an expression of thanks
>    - → **表情，神色**：把内心情感压出来留在脸上的痕迹
>      - 激活条件：facial expression, a sad expression
>    - → **措辞，表达方式**：把想法压出来所使用的具体语言形式
>      - 激活条件：a polite expression, colloquial expressions

### 统筹（选择适用的模式）

%% HERMES-L1: 根据该词的一词多义特征，选择最合适的统筹模式 %%
%% HERMES-L1: 统筹模式参考哲学文档2.3节，以下6种模式选1-2种即可 %%

- **一字一词概括**：expression 就是"压出来的东西"——语言是压出来的表达，表情是压出来的神色，措辞是压出来的形式
- **一句话统筹**：一切 expression 都是从内向外的"挤压产物"

> [!check]- 链路验证
> - [x] **可逆性**：从"表情"能反向推导到"挤压出来的结果"——表情就是内心情感压出来留在脸上的痕迹
> - [x] **可统筹**：一句话——"expression 就是从内向外的挤压产物"
> - [x] **有支撑**：freedom of expression, facial expression, polite expression 均有语料支撑

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 表达；表情；措辞 | 核心词性，[[express]] 的名词形式（压出来的结果） |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

%% HERMES-L1: 分析词性转换路径(哲学文档2.4节) %%
%% HERMES-L1: v→n: 该动作的结果是(什么) / n→n: 子集链路/特性抽象 %%
%% HERMES-L1: a→v: 使动用法 / v→a: +ed表示"持有-动作"的状态 %%

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> [[express]] 是把想法"压出来"，[[expression]] 就是压出来的结果。你的面部表情（facial expression）就是内心情感压出来后留在脸上的痕迹——开心时嘴角上扬，悲伤时眉头紧锁，都是内心被"压"到脸上的 expression。freedom of expression 就是让你自由地把内心"压出来"的权利。
>
> **词拆分记忆**（辅助）：
> ex（出）+ press（压）+ -ion（名词后缀）→ 向外压出来的结果 → 表达/表情

%% HERMES-L1: 优先使用叙事化词源(哲学文档2.7.4)，其次词拆分记忆(2.6节)，再次双编码策略(3.2节) %%
%% HERMES-L1: 词拆分允许创造性拆分(如 contemplate = con + template + plate) %%

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[statement]] | 声明，陈述 | 正式、书面化的表达 | 官方立场、政策 | 正式/权威 |
> | [[utterance]] | 话语，言辞 | 强调说出的话本身 | 言语、声音 | 中性/学术 |
> | [[manifestation]] | 表现，显现 | 强调内在本质的外在显示 | 意志、态度、症状 | 正式/深层 |
> | **expression** | 表达；表情；措辞 | 强调从内到外的挤压产物（最通用） | 情感、想法、面部 | 通用/中性 |
>
> **"增"标记**：expression **(增-面部表情维度)** vs statement（基准词：正式声明）；expression **(增-通用性和日常感)** vs manifestation（基准词：深层显现）

%% HERMES-L2: 四维辨析法(哲学文档3.4节) %%
%% HERMES-L2: "增"标记法(哲学文档2.7.1): 标记相对基准词的语义/情感增量 %%
%% HERMES-L2: 检查 Wiki/同义辨析/ 是否已有对应辨析笔记，有则[[链接]] %%

## 反义词

> [!note]- 反义词 · L2
> - [[suppression]]：压制，抑制（expression 的反义：压回去 vs 压出来）
> - [[concealment]]：隐藏，隐瞒（expression 的反义：隐藏起来 vs 表达出来）

%% HERMES-L2: 添加[[wikilink]]，说明反义关系 %%

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[express]] | ex-(向外) + press(压) | 表达；快递 | 词义补充（expression 的动词形式） |
> | [[expressive]] | express + -ive(倾向…的) | 富于表现的，有表现力的 | 词义补充 |
> | [[expressly]] | express + -ly(副词后缀) | 明确地，特地 | 词义补充 |
> | [[expressionless]] | expression + -less(没有) | 没有表情的 | 词义补充 |

%% HERMES-L2: 派生词链接(哲学文档2.5节) %%
%% HERMES-L2: 词义补充: 派生词反哺母词 / 词义填充: 母词填充派生词 %%

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **freedom of expression**：言论自由，表达自由
> - **facial expression**：面部表情
> - **an expression of concern/regret/sympathy**：关切/遗憾/同情的表示
> - **give expression to**：表达，抒发
> - **beyond expression**：无法用语言表达

%% HERMES-L3: 仅列考研真题高频搭配，不编造 %%
%% HERMES-L3: 每个搭配附带1个真题例句 %%

## 真题/语料关联

> [!example]- 语料 · L3
> - "It is speculated that gardens arise from a basic need in the individuals who made them: the need for creative expression."（人们推测，花园源于建造者的一种基本需求：创造性表达的需求。——2013年考研英语一翻译）
> - "He advocated freedom of thought and of personal expression."（他倡导思想自由和个人言论自由。——2014年考研英语一翻译）

%% HERMES-L3: 链接到阅读/翻译笔记中的具体段落 %%

---

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-22 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

%% 复习间隔参考 [[词汇学习哲学#与加工深度等级的对应]] %%

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>

%% HERMES-L4: 仅在用户实际产出后填写 %%

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：press（压、挤压，拉丁语 *premere*）
> - [ ] 同义辨析：expression/statement/utterance/manifestation
> - [ ] 反义词群：suppression/concealment
> - [ ] 形近词群：express/expression/expressive/expressly

%% HERMES-ACTIVATION: AI 根据用户回复勾选对应 checkbox，/回顾时读取勾选状态决定是否纳入对应网络 %%
