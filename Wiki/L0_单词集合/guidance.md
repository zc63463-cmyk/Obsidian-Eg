---
title: "guidance"
tags:
  - 学习/英语/词汇
  - 语义场/社会专业
  - 掌握/L0
aliases: []
date: 2026-04-21
mastery: L0
word_freq: 高频
semantic_field: 社会专业
prototype: 引导的行为或过程
extension_dim: 抽象路径
last_review: 2026-04-21
review_count: 0
---

# guidance

> [!info] 基础信息
> **音标** /ˈɡaɪdns/ | **词频** 考研高频 | **语义场** [[社会专业]]

## 核心释义

**n.** ①==**指导；引导**==；②咨询；

> [!tip] 原型义
> **原型义**：引导（guide）的行为或过程
> **延伸维度**：抽象路径
> **隐喻类型**：无隐喻（直接从动词抽象为名词）

## 词根词缀

[[guide]]（引导）+ -ance（名词后缀，表示行为/状态）
**叙事**：引导的过程和结果，就是指导。

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**引导的行为或过程**"为统筹中心，沿抽象路径向外扩展：
>
> 1. **引导的行为或过程**（行为层面）
>    - → **指导**（教育/职业层面：系统性的引导）
>      - 激活条件：搭配 career/parental/academic guidance 时
>    - → **咨询**（专业层面：专家提供的引导建议）
>      - 激活条件：搭配 guidance counselor/seek guidance 时

### 统筹（选择适用的模式）

- **一句话统筹**：引导的行为或过程——指导/咨询

> [!check]- 链路验证
> - [x] **可逆性**：指导 = 引导的行为过程，可逆推
> - [x] **可统筹**："引导的行为或过程"串联所有义项
> - [x] **有支撑**：career guidance（职业指导）/ seek guidance from（寻求...的指导）

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 指导；引导 | 基本词性 |
> | v. (guide) | 引导；指导 | n→v（去掉-ance） |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> 老师在迷雾中为学生指路，这个"指路的过程"就是 guidance——指导。
>
> **词拆分记忆**（辅助）：
> guide（引导）+ -ance（名词后缀）→ 引导的行为 → 指导

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[instruction]] | 教导 | 系统传授 | 学生/课程 | 正式 |
> | [[direction]] | 指示 | 给出方向 | 操作/任务 | 中性 |
> | [[counsel]] | 建议 | 深度专业建议 | 法律/心理 | 正式 |
> | **guidance** | 指导 | 陪伴式引导 | 职业/学术 | 积极 |
>
> **"增"标记**：guidance **(增-持续性与陪伴感)** vs direction

## 反义词

> [!note]- 反义词 · L2
> - [[misdirection]]：误导——错误的引导

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[guide]] | 基词 | 引导；导游 | 词义填充 |
> | [[guideline]] | guide + line | 指导方针 | 词义补充 |

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **under the guidance of**：Under the guidance of her mentor, she completed the research successfully.（在导师的指导下，她成功完成了研究。）
> - **career guidance**：The university provides career guidance for graduating students.（大学为毕业生提供职业指导。）
> - **seek guidance**：You should seek guidance from a professional.（你应该寻求专业人士的指导。）

## 真题/语料关联

> [!example]- 语料 · L3
> - "Children need proper guidance from their parents and teachers to develop good study habits."（儿童需要父母和老师的正确指导来养成良好的学习习惯。）——考研高频表达
> - "The government provides financial guidance to help young people manage their personal finances."（政府提供财务指导，帮助年轻人管理个人财务。）——考研高频表达


---

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-21 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：<待填写>
> **翻译实践**：<待填写>

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：无对应词根笔记
> - [ ] 同义辨析：instruction, direction, counsel
> - [ ] 反义词群：misdirection
> - [ ] 形近词群：guide, guard, guild
