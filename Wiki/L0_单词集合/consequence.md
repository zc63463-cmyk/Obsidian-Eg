---
title: "consequence"
tags:
  - 学习/英语/词汇
  - 语义场/抽象关系
aliases: []
date: 2026-04-21
word_freq: 必备词
semantic_field: 抽象关系
prototype: 跟随而来的结果
extension_dim: [时间路径]
phonetic: "/ˈkɒnsɪkwəns/"
pos: n.
word_root: secut
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-21
review_count: 0
---

# consequence

> [!info] 基础信息
> **音标** /ˈkɒnsɪkwəns/ | **词级** 必备词 | **语义场** [[抽象关系]]

## 核心释义

**n.** ①==**结果，后果**== `as a N` `suffer the Ns` ; ②==**重要性，重大意义**== `of N` `of little/no N` ;

> [!tip] 原型义
> **原型义**：跟随而来的结果（一件事紧随另一件事发生）
> **延伸维度**：时间路径
> **隐喻类型**：无隐喻（直接从时间先后关系抽象而来）

## 词根词缀

con-(共同，一起) + sequ/secut(跟随) + -ence(名词后缀)
建议创建词根笔记 [[sequ-secut]]
**叙事**：con-（一起）+ sequ-（跟随）+ -ence（名词）→ 紧随其后的东西 → 结果、后果

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**跟随而来的结果**"为统筹中心，沿时间路径向外扩展：
>
> 1. **跟随而来的结果**（时间层面）
>    - → **结果，后果**：一件事紧随另一件事发生
>      - 激活条件：描述某行为或事件导致的结果（常带消极色彩）
>    - → **重要性，重大意义**：从"重要的后果"引申为"重要性本身"
>      - 激活条件：正式语境中描述某事的重要程度

### 统筹（选择适用的模式）

- **一字一词概括**：随——跟随而来的就是后果
- **延伸中心**：以"跟随"为核心，从时间先后延伸到因果关联
- **一句话统筹**：跟着来的——后果→重要性

> [!check]- 链路验证
> - [x] **可逆性**：从"重要性"可反向推导到"有重大后果的"
> - [x] **可统筹**：所有义项都围绕"跟随/结果"这一核心
> - [x] **有支撑**：as a consequence（因此）、of little consequence（无关紧要）

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 结果，重要性 | 基本词性 |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> sequ-（跟随）→ 想象你推倒了一排多米诺骨牌，最后一块倒下的就是 consequence（后果）。
>
> **词拆分记忆**：
> con（共同）+ sequence（顺序）→ 共同的顺序中跟在后面的 → 后果

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[result]] | 结果 | 强调因果关系的产出 | 实验、比赛、努力 | 中性 |
> | [[outcome]] | 结果，结局 | 强调最终状态 | 谈判、选举、治疗 | 中性 |
> | [[effect]] | 效果，影响 | 强调作用产生的变化 | 政策、药物、环境 | 中性 |
> | **consequence** | 后果，重要性 | 强调跟随而来的（常消极） | 行为、决定、事件 | 偏消极/正式 |
>
> **"增"标记**：consequence **(增-消极色彩)** vs result：consequence 更常带消极色彩，暗示不良后果

## 反义词

> [!note]- 反义词
> - [[cause]]：原因（consequence 描述的是"果"，cause 是"因"）
> - [[origin]]：起源（结果 vs 起源）

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[consequently]] | consequence + -ly(副词后缀) | 因此，所以 | 词义填充 |
> | consequent | con+sequ+ent(形容词后缀) | 作为结果的 | 词义补充 |
> | sequence | sequ(跟随)+-ence(名词后缀) | 顺序，序列 | 同根词义补充 |
> | [[conservation]] | con+serv(保持)+-ation | 保护，保存 | con-族同前缀词 |

%% consequently、conservation 为本批次同族词 %%

---

## 搭配与短语

> [!example]- 搭配
> - **as a consequence**：因此，结果（As a consequence, many species are now endangered.）
> - **in consequence of**：由于（He lost his job in consequence of his careless behavior.）
> - **of consequence**：重要的（It is a matter of no small consequence.）
> - **suffer the consequences**：承担后果（You must be prepared to suffer the consequences.）

## 真题/语料关联

> [!example]- 语料
> - It comes at a time . when authors are already worried about the consequences of the proposed merger between PRH and another big publisher , Simon & Schuster-the reduction in the number of unaligned UK publishers is likely to lead to fewer bidding wars,, lower advances, and more conformity in . terms of what is published. `[真题]`
>   - 中译：它来得正是时候。当作者们已经担心 PRH 与另一家大型出版商西蒙与舒斯特 (Simon & Schuster) 之间拟议合并的后果时，不结盟的英国出版商数量的减少可能会导致竞标战的减少、预付款的降低以及 .所发布内容的条款。
>   - 来源：考研-2023-英语一 | http://gw.xztu.edu.cn/__local/D/8F/1D/8F556EF355DAB6127D7D2D4048F_BD575BBC_DE37A.pdf
> - What can be one consequence of the current dispute? `[真题]`
>   - 中译：当前争端可能会产生什么后果？
>   - 来源：考研-2023-英语一 | http://gw.xztu.edu.cn/__local/D/8F/1D/8F556EF355DAB6127D7D2D4048F_BD575BBC_DE37A.pdf
> - Not making a will can have serious consequences for your children and other family members. `[例]`
>   - 中译：不立遗嘱可能会给您的孩子和其他家庭成员带来严重后果。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/consequence
---


## 主动产出

> [!success]- 主动产出
