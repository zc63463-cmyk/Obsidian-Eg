---
title: "ambition"
tags:
  - 学习/英语/词汇
  - 语义场/人本世界
  - 掌握/L0
aliases: []
date: 2026-04-22
mastery: L0
word_freq: 高频
semantic_field: 人本世界
prototype: 四处走动争取选票、追求目标
extension_dim: 社会路径
last_review: 2026-04-22
review_count: 0
---

# ambition

> [!info] 基础信息
> **音标** /æmˈbɪʃn/ | **词频** 考研高频 | **语义场** [[人本世界]]

## 核心释义

**n.** ①==**抱负，雄心**== ; ②野心，奢望 ; ③追求的目标

> [!tip] 原型义
> **原型义**：四处走动争取支持（古罗马候选人拉票）
> **延伸维度**：社会路径
> **隐喻类型**：结构隐喻——从"四处奔走争取选票"投射到"追求远大目标"

## 词根词缀

ambi-(周围、四处) + -ition(名词后缀，来自 ire/it，走) → 四处走动 → 争取选票 → 追求目标 → 抱负/野心
**叙事**：古罗马的政治候选人**四处走动**（ambi + it）拉票，这种对权力的**追求**就是 **ambition**。追求正当目标就是**抱负**，追求不当目标就是**野心**。

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**四处奔走追求目标**"为统筹中心，沿社会路径向外扩展：
>
> 1. **四处奔走追求目标**（行为层面：积极争取）
>    - → **抱负，雄心**：<对成就的正当追求>
>      - 激活条件：搭配 achieve, fulfill，搭配个人发展语境
>    - → **野心，奢望**：<对权力/地位的不当追求>
>      - 激活条件：搭配 power, control，搭配批评语境
>    - → **追求的目标**：<追求的对象本身>
>      - 激活条件：搭配 lifelong, political

### 统筹（选择适用的模式）

- **延伸中心**："追求"→ 追求成就（抱负）→ 追求权力（野心）→ 追求的对象（目标）
- **一句话统筹**：ambition 是驱动一个人四处奔走的内在追求，可褒可贬

> [!check]- 链路验证
> - [x] **可逆性**：从"抱负"可反向推导——四处奔走追求 → 追求目标
> - [x] **可统筹**：所有义项都围绕"追求目标"这一核心
> - [x] **有支撑**：ambition and determination / political ambition 为考研高频用法

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 抱负；野心；目标 | 原型义：追求目标的行为/心理 |
> | adj. (ambitious) | 有抱负的；有野心的 | n→adj：+ous 表示"充满……的" |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**：
> 想象一个古罗马政治家，**四处走动**（ambi + it）发表演讲、争取选票——他对权力的强烈**追求**就是 **ambition**。今天我们说一个人有 ambition，可能是在夸他有**抱负**，也可能是在说他有**野心**，取决于他追求的是什么。
>
> **词拆分记忆**：
> ambition = ambi-(四处) + -ition(走) → 四处走动争取 → 抱负/野心

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[aspiration]] | 渴望，抱负 | 偏理想主义 | 理想、目标 | 积极 |
> | [[goal]] | 目标 | 通用，具体 | 任何目标 | 中性 |
> | [[desire]] | 欲望，渴望 | 通用 | 任何渴望 | 中性 |
> | **ambition** | 抱负；野心 | 强烈追求，可褒可贬 | 成就、权力 | 积极/消极 |
>
> **"增"标记**：ambition **(增-强烈程度+双面性)** vs aspiration——ambition 更强烈，且可褒可贬

## 反义词

> [!note]- 反义词 · L2
> - [[indifference]]：ambition（抱负）vs indifference（冷漠）
> - [[complacency]]：ambition（进取心）vs complacency（自满）

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[ambitious]] | ambition + -ous | 有抱负的；有野心的 | 词义填充 |
> | [[ambiguous]] | ambi-(两边) + ig(走) | 模棱两可的 | 词义补充 |
> | unambitious | un- + ambitious | 没有抱负的 | 词义补充 |

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **achieve/fulfill one's ambition**：实现某人的抱负（She finally achieved her ambition of becoming a doctor. 她终于实现了成为医生的抱负。）
> - **political ambition**：政治野心（His political ambition was evident from an early age. 他的政治野心从很小的时候就显而易见了。）
> - **burning ambition**：强烈的抱负（He has a burning ambition to succeed in business. 他对在商业上取得成功有着强烈的抱负。）

## 真题/语料关联

> [!example]- 语料 · L3
> - "Her lifelong ambition was to become a renowned scientist."（她毕生的追求是成为一名著名的科学家。）——2021考研英语阅读Text3
> - "Political ambition can drive leaders to achieve great things, but it can also lead to corruption."（政治野心可以驱使领导人成就伟业，但也可能导致腐败。）——考研高频表达
> - "He fulfilled his ambition of climbing Mount Everest before the age of thirty."（他在三十岁之前实现了攀登珠穆朗玛峰的抱负。）——考研高频表达

---

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-22 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：<待填写>
> **翻译实践**：<待填写>

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 同义辨析：ambition / aspiration / goal / desire
> - [ ] 反义词群：ambition ↔ indifference / complacency
> - [ ] 形近词群：ambition / ambitious / ambiguous / ambient
