---
title: "extend"
tags:
  - 学习/英语/词汇
  - 语义场/自然物理
aliases: []
date: 2026-04-22
word_freq: 必备词
semantic_field: 自然物理
prototype: 向外伸展（从中心向外拉长）
extension_dim: [空间路径]
phonetic: "/ɪkˈstend/"
pos: v.
metaphor_type: 方位隐喻
word_root: tend-tens
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-22
review_count: 0
---

# extend

> [!info] 基础信息
> **音标** /ɪkˈstend/ | **词级** 必备词 | **语义场** [[自然物理]]

## 核心释义

**v.** ①==**延伸，延长**== `V N` `V N to N`；②==**扩大，扩展**== `V N`；③给予，提供（帮助、欢迎等） `V N to N`；④伸展（身体） `V N`；

> [!tip] 原型义
> **原型义**：向外伸展（从中心向外拉长、伸展出去）
> **延伸维度**：空间路径
> **隐喻类型**：方位隐喻（空间上的向外伸展→时间上的延长、范围上的扩大）

## 词根词缀

ex-(=out, 向外) + [[tend-tens|tend]](=伸展，拉丁语 *tendere*)
**叙事**：ex（向外）+ tend（伸展）→ **向外伸展**。一条路从城市中心向外伸展（extend the road），一个项目的截止日期向外伸展（extend the deadline），你的知识面从专业向外伸展到跨学科（extend one's knowledge）。

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**向外伸展**"为统筹中心，沿空间路径向外扩展：
>
> 1. **向外伸展**（空间层面）
>    - → **延伸，延长**：在空间或时间上向外伸展
>      - 激活条件：extend the deadline, extend the road
>    - → **扩大，扩展**：在范围或规模上向外伸展
>      - 激活条件：extend one's knowledge, extend influence
>    - → **给予，提供**：把手/善意向外伸展给他人
>      - 激活条件：extend a warm welcome, extend an invitation

### 统筹（选择适用的模式）

- **一字一词概括**：extend 就是"向外伸"——伸长是时间/空间上的延伸，伸手是给予/提供
- **一句话统筹**：一切 extend 都是从中心向外的"伸展"

> [!check]- 链路验证
> - [x] **可逆性**：从"给予欢迎"能反向推导到"向外伸展"——extend a welcome 就是把善意向外伸给对方
> - [x] **可统筹**：一句话——"extend 就是从中心向外的伸展"
> - [x] **有支撑**：extend the deadline, extend knowledge, extend a welcome 均有语料支撑

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | v. | 延伸；扩大；给予 | 核心词性，"向外伸展"的动作 |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> ex（向外）+ tend（伸展）→ 向外伸展。想象你站在一个圆的中心，向四面八方伸展开来——你的手臂向外伸展（extend your arms），你的影响力向外伸展（extend your influence），你的截止日期向外伸展（extend the deadline）。extend 的灵魂就是"从中心向外"。
>
> **词拆分记忆**（辅助）：
> ex（向外）+ tend（伸展）→ 向外伸展 → 延伸/扩大

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[expand]] | 扩大，膨胀 | 强调体积/面积的膨胀变大 | 业务、经济、规模 | 中性/积极 |
> | [[stretch]] | 拉伸，伸展 | 强调物理上的拉长，可能超过极限 | 绳子、肌肉、资源 | 中性/可能消极 |
> | [[prolong]] | 延长 | 专指时间上的延长 | 寿命、战争、痛苦 | 中性 |
> | **extend** | 延伸，扩大 | 强调从中心向外的伸展（最通用） | 范围、时间、影响 | 通用/中性 |
>
> **"增"标记**：extend **(增-空间伸展感+给予义)** vs expand（基准词：膨胀扩大）；extend **(增-通用性，可指时间/空间/范围)** vs prolong（基准词：时间延长）

## 反义词

> [!note]- 反义词
> - [[shorten]]：缩短（extend 的反义：向外伸展 vs 向内缩短）
> - [[shrink]]：收缩，缩小（extend 的反义：向外扩展 vs 向内收缩）
> - [[reduce]]：减少，缩小（extend 的反义：扩大 vs 减少）

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[extension]] | extend + -ion(名词后缀) | 延伸；扩展；延期 | 词义填充（extend 的名词形式） |
> | [[extensive]] | extend + -sive(倾向…的) | 广泛的，大量的 | 词义补充 |
> | [[extent]] | ex-(向外) + tent(=tend-, 伸展) | 程度；范围 | 词义补充（同源词） |
> | [[extended]] | extend + -ed(形容词后缀) | 延长的，持续的 | 词义补充 |

---

## 搭配与短语

> [!example]- 搭配
> - **extend the deadline**：延长截止日期
> - **extend one's knowledge/influence**：扩展知识/影响力
> - **extend a warm welcome to**：向……致以热烈欢迎
> - **extend from A to B**：从A延伸到B
> - **extend over**：延续（一段时间）

## 真题/语料关联

> [!example]- 语料
> - We have plans to extend our house (= to make it bigger ). `[例]`
>   - 中译：我们计划扩建我们的房子（=让它变得更大）。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/extend
> - The government has produced a series of leaflets designed to extend (= increase ) public awareness of the dangers of AIDS. `[例]`
>   - 中译：政府制作了一系列传单，旨在提高（=提高）公众对艾滋病危险的认识。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/extend
> - We're planning to extend our publishing of children's books (= increase it). `[例]`
>   - 中译：我们计划扩大儿童读物的出版范围（=增加）。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/extend

---


## 主动产出

> [!success]- 主动产出
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>
