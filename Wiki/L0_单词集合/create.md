---
title: "create"
tags:
  - 学习/英语/词汇
  - 语义场/行为动作
  - 掌握/L0
aliases: []
date: 2026-04-22
mastery: L0
word_freq: 高频
semantic_field: 行为动作
prototype: 增长（使事物从无到有地增长出来）
extension_dim: 时间路径
last_review: 2026-04-22
review_count: 0
---

# create

> [!info] 基础信息
> **音标** /kriˈeɪt/ | **词频** 考研高频 | **语义场** [[行为动作]]

%% HERMES-L0: 填写音标(IPA)、词频(考研高/中/低频)、判定语义场 %%
%% HERMES-L0: 语义场判定规则见 SKILL.md Step 3 %%

## 核心释义

%% HERMES-L0: 按词性分类，仅列考研常考义项(不超过4个)，使用紧凑格式 %%
%% HERMES-L0: 格式: **词性.** ①释义1 ; ②释义2 ; 考研重点义项用 ==**加粗+高亮**== 标记 %%

**v.** ①==**创造，创作**==；②==**引起，产生**==；③设立，建立；

> [!tip] 原型义
> **原型义**：增长（使事物从无到有地增长出来）
> **延伸维度**：时间路径
> **隐喻类型**：本体隐喻（"增长"→"创造"——从无到有的过程就是创造）

%% HERMES-L0: 判定原型义(物理动作优先>最具体义项优先>最早学到优先>词源义参考) %%
%% HERMES-L0: 判定主延伸维度(使用频率最高的延伸路径) %%
%% HERMES-L0: 判定隐喻类型(哲学文档2.1节): 方位/结构/本体 %%

## 词根词缀

%% HERMES-L0: 拆分前缀+词根+后缀，检查已有词根笔记并添加[[链接]] %%
%% HERMES-L0: 检查路径: VocabVault/Wiki/词根词缀/ %%

cre(=cres-, 增长，拉丁语 *crescere*) + -ate(动词后缀，使……)
**叙事**：cre（增长）+ -ate（使）→ 使事物增长出来 → **创造**。从一颗种子"增长"成一棵树，就是 create 的原始画面。上帝 create 世界，就是让世界从无到有地"增长"出来。建议创建词根笔记 [[cre-cresc-cret]]（增长，拉丁语 *crescere*）。

%% HERMES-L0: 叙事记忆创作参考哲学文档2.7.4节 %%
%% HERMES-L0: 如果词根笔记不存在，在输出中提示"建议创建词根笔记 [[<词根>]]" %%

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**增长（使从无到有）**"为统筹中心，沿时间路径向外扩展：
>
> 1. **增长**（从无到有的过程）
>    - → **创造，创作**：使新事物从无到有地产生
>      - 激活条件：create art, create a new method
>    - → **引起，产生**：使新情况从无到有地出现
>      - 激活条件：create jobs, create problems
>    - → **设立，建立**：使新机构从无到有地建立
>      - 激活条件：create a committee, create a system

### 统筹（选择适用的模式）

%% HERMES-L1: 根据该词的一词多义特征，选择最合适的统筹模式 %%
%% HERMES-L1: 统筹模式参考哲学文档2.3节，以下6种模式选1-2种即可 %%

- **一字一词概括**：create 就是"从无到有"——从无到有地产生艺术=创造，从无到有地产生问题=引起
- **v + 不同宾语**：create art（创作）→ create jobs（创造就业）→ create problems（引起问题）

> [!check]- 链路验证
> - [x] **可逆性**：从"引起问题"能反向推导到"使问题从无到有地产生"
> - [x] **可统筹**：一句话——"create 就是使事物从无到有"
> - [x] **有支撑**：create jobs, create problems, create art 均有语料支撑

%% HERMES-L1: 应用词义链路法操作规程(哲学文档2.2节) %%
%% HERMES-L1: Step1判定原型义 → Step2确定延伸维度+隐喻类型 → Step3描述路径+激活条件 → Step4选择统筹模式 %%
%% HERMES-L1: 验证标准: 可逆性+可统筹+有支撑 %%

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | v. | 创造；引起；设立 | 核心词性，"从无到有"的动作 |
> | n. | 创造物 | v→n：从无到有产生的东西 |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

%% HERMES-L1: 分析词性转换路径(哲学文档2.4节) %%
%% HERMES-L1: v→n: 该动作的结果是(什么) / n→n: 子集链路/特性抽象 %%
%% HERMES-L1: a→v: 使动用法 / v→a: +ed表示"持有-动作"的状态 %%

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> cre（增长）+ -ate（使）→ 使增长出来 → 创造。想象一颗种子在土壤中慢慢"增长"，最终破土而出——这就是 create 的原始画面。从一颗种子到一棵大树，从一张白纸到一幅画，从零到一个企业——create 就是"使从无到有"。
>
> **词拆分记忆**（辅助）：
> cre（生长）+ ate（吃过去式）→ 吃了就生长 → 创造

%% HERMES-L1: 优先使用叙事化词源(哲学文档2.7.4)，其次词拆分记忆(2.6节)，再次双编码策略(3.2节) %%
%% HERMES-L1: 词拆分允许创造性拆分(如 contemplate = con + template + plate) %%

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[produce]] | 生产，制造 | 强调批量生产 | 产品、商品 | 中性 |
> | [[invent]] | 发明 | 强调首创 | 技术、方法 | 积极 |
> | [[generate]] | 产生，生成 | 强调产生结果 | 收入、能量 | 中性 |
> | **create** | 创造；引起 | 强调"从无到有"（最通用） | 艺术、就业、问题 | 积极/通用 |
>
> **"增"标记**：create **(增-从无到有的原创性)** vs produce（基准词：批量生产）；create **(增-可指负面结果)** vs invent（基准词：仅正面发明）

%% HERMES-L2: 四维辨析法(哲学文档3.4节) %%
%% HERMES-L2: "增"标记法(哲学文档2.7.1): 标记相对基准词的语义/情感增量 %%
%% HERMES-L2: 检查 Wiki/同义辨析/ 是否已有对应辨析笔记，有则[[链接]] %%

## 反义词

> [!note]- 反义词 · L2
> - [[destroy]]：毁灭（create 的精确反义：创造↔毁灭）
> - [[eliminate]]：消除（create 的反义：产生↔消除）

%% HERMES-L2: 添加[[wikilink]]，说明反义关系 %%

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[creative]] | cre(增长) + -at(e) + -ive(倾向) | 有创造力的 | 词义填充（create 的形容词形式） |
> | [[creation]] | cre(增长) + -ation(名词后缀) | 创造，创造物 | 词义补充 |
> | [[creativity]] | creative + -ity(名词后缀) | 创造力 | 词义补充 |
> | [[creature]] | cre(增长) + -ature(名词后缀) | 生物，动物 | 词义补充（被创造出来的东西） |
> | [[credit]] | cre(增长/信任) + -dit(给予) | 信用；功劳 | 同词根族 |

%% HERMES-L2: 派生词链接(哲学文档2.5节) %%
%% HERMES-L2: 词义补充: 派生词反哺母词 / 词义填充: 母词填充派生词 %%

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **create jobs**：创造就业
> - **create problems**：引起问题
> - **create wealth**：创造财富
> - **create opportunities**：创造机会

%% HERMES-L3: 仅列考研真题高频搭配，不编造 %%
%% HERMES-L3: 每个搭配附带1个真题例句 %%

## 真题/语料关联

> [!example]- 语料 · L3
> - "Instead, the new habits we deliberately ingrain into ourselves create parallel pathways that can bypass those old roads."（相反，我们有意使之根深蒂固的新习惯会创建平行路径，它们可以绕过原来那些路径。——2009年考研英语阅读Text1）
> - "It is speculated that gardens arise from a basic need in the individuals who made them: the need for creative expression. There is no doubt that gardens evidence an impossible urge to create, express, fashion, and beautify."（据推测，花园源于建造者的一种基本需要：创造性表达的需要。毫无疑问，花园证明了一种不可抗拒的创造、表达、塑造和美化的冲动。——2013年考研英语翻译）

%% HERMES-L3: 链接到阅读/翻译笔记中的具体段落 %%

---

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-22 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

%% 复习间隔参考 [[词汇学习哲学#与加工深度等级的对应]] %%

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>

%% HERMES-L4: 仅在用户实际产出后填写 %%

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：cre-/cresc-/cret-（增长，拉丁语 *crescere*）
> - [ ] 同义辨析：create/produce/invent/generate
> - [ ] 反义词群：destroy/eliminate
> - [ ] 形近词群：create/creative/creation/creativity/creature/credit

%% HERMES-ACTIVATION: AI 根据用户回复勾选对应 checkbox，/回顾时读取勾选状态决定是否纳入对应网络 %%
