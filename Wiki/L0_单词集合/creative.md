---
title: "creative"
tags:
  - 学习/英语/词汇
  - 语义场/行为态度
  - 掌握/L0
aliases: []
date: 2026-04-22
mastery: L0
word_freq: 高频
semantic_field: 行为态度
prototype: 倾向于增长的（倾向于从无到有地创造）
extension_dim: 时间路径
last_review: 2026-04-22
review_count: 0
---

# creative

> [!info] 基础信息
> **音标** /kriˈeɪtɪv/ | **词频** 考研高频 | **语义场** [[行为态度]]

%% HERMES-L0: 填写音标(IPA)、词频(考研高/中/低频)、判定语义场 %%
%% HERMES-L0: 语义场判定规则见 SKILL.md Step 3 %%

## 核心释义

%% HERMES-L0: 按词性分类，仅列考研常考义项(不超过4个)，使用紧凑格式 %%
%% HERMES-L0: 格式: **词性.** ①释义1 ; ②释义2 ; 考研重点义项用 ==**加粗+高亮**== 标记 %%

**adj.** ①==**有创造力的，创造性的**==；②富有想象力的；
**n.** ①创意工作者（非正式）；

> [!tip] 原型义
> **原型义**：倾向于增长的（倾向于从无到有地创造新事物）
> **延伸维度**：时间路径
> **隐喻类型**：本体隐喻（"从无到有的倾向"→"有创造力的"——具备使事物增长的能力）

%% HERMES-L0: 判定原型义(物理动作优先>最具体义项优先>最早学到优先>词源义参考) %%
%% HERMES-L0: 判定主延伸维度(使用频率最高的延伸路径) %%
%% HERMES-L0: 判定隐喻类型(哲学文档2.1节): 方位/结构/本体 %%

## 词根词缀

%% HERMES-L0: 拆分前缀+词根+后缀，检查已有词根笔记并添加[[链接]] %%
%% HERMES-L0: 检查路径: VocabVault/Wiki/词根词缀/ %%

cre(=cres-, 增长，拉丁语 *crescere*) + -at(e)(动词后缀) + -ive(倾向……的)
**叙事**：cre（增长）+ -ate（使）+ -ive（倾向……的）→ 倾向于使事物增长出来的 → **有创造力的**。[[create]] 是"从无到有"的动作，[[creative]] 是"倾向于从无到有"的特质。一个 creative 的人就像一颗种子，总能让新想法从无到有地"增长"出来。建议创建词根笔记 [[cre-cresc-cret]]（增长，拉丁语 *crescere*）。

%% HERMES-L0: 叙事记忆创作参考哲学文档2.7.4节 %%
%% HERMES-L0: 如果词根笔记不存在，在输出中提示"建议创建词根笔记 [[<词根>]]" %%

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**倾向于增长（从无到有）**"为统筹中心，沿时间路径向外扩展：
>
> 1. **倾向于增长**（能力层面）
>    - → **有创造力的**：具备从无到有创造新事物的能力
>      - 激活条件：creative ideas, creative thinking
>    - → **富有想象力的**：思维活跃，能产生新颖想法
>      - 激活条件：creative approach, creative solution

### 统筹（选择适用的模式）

%% HERMES-L1: 根据该词的一词多义特征，选择最合适的统筹模式 %%
%% HERMES-L1: 统筹模式参考哲学文档2.3节，以下6种模式选1-2种即可 %%

- **一字一词概括**：creative 就是"倾向于从无到有"——从无到有的想法=有创造力的
- **延伸中心**：从"倾向于增长"延伸到"有创造力的"和"富有想象力的"

> [!check]- 链路验证
> - [x] **可逆性**：从"有创造力的"能反向推导到"倾向于从无到有地创造"
> - [x] **可统筹**：一句话——"creative 就是倾向于从无到有"
> - [x] **有支撑**：creative ideas, creative thinking 均有语料支撑

%% HERMES-L1: 应用词义链路法操作规程(哲学文档2.2节) %%
%% HERMES-L1: Step1判定原型义 → Step2确定延伸维度+隐喻类型 → Step3描述路径+激活条件 → Step4选择统筹模式 %%
%% HERMES-L1: 验证标准: 可逆性+可统筹+有支撑 %%

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | adj. | 有创造力的 | 核心词性，create 的形容词形式 |
> | n. | 创意工作者 | adj→n：有创造力的人 |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

%% HERMES-L1: 分析词性转换路径(哲学文档2.4节) %%
%% HERMES-L1: v→n: 该动作的结果是(什么) / n→n: 子集链路/特性抽象 %%
%% HERMES-L1: a→v: 使动用法 / v→a: +ed表示"持有-动作"的状态 %%

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> create（从无到有）+ -ive（倾向……的）→ 倾向于从无到有的 → 有创造力的。2018年考研阅读提到教育应 focus less on memorizing facts and more on creativity and complex communication——少记事实，多培养创造力和复杂沟通能力。creative 的人就像一块肥沃的土壤，总能让新想法从无到有地"生长"出来。
>
> **词拆分记忆**（辅助）：
> cre（生长）+ at（在）+ ive（活着）→ 在活着的时候生长 → 有创造力的

%% HERMES-L1: 优先使用叙事化词源(哲学文档2.7.4)，其次词拆分记忆(2.6节)，再次双编码策略(3.2节) %%
%% HERMES-L1: 词拆分允许创造性拆分(如 contemplate = con + template + plate) %%

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[imaginative]] | 富有想象力的 | 强调想象力的丰富 | 故事、方案 | 积极 |
> | [[innovative]] | 创新的 | 强调引入新方法 | 技术、管理 | 积极 |
> | [[original]] | 独创的 | 强调前所未有的 | 想法、作品 | 积极 |
> | **creative** | 有创造力的 | 强调从无到有的能力（最通用） | 想法、工作、人 | 积极/通用 |
>
> **"增"标记**：creative **(增-从无到有的过程感)** vs imaginative（基准词：仅想象）；creative **(增-通用性)** vs innovative（基准词：仅创新）

%% HERMES-L2: 四维辨析法(哲学文档3.4节) %%
%% HERMES-L2: "增"标记法(哲学文档2.7.1): 标记相对基准词的语义/情感增量 %%
%% HERMES-L2: 检查 Wiki/同义辨析/ 是否已有对应辨析笔记，有则[[链接]] %%

## 反义词

> [!note]- 反义词 · L2
> - [[uncreative]]：缺乏创造力的（creative 的派生反义）
> - [[routine]]：常规的（creative 的反义：创造性↔常规性）

%% HERMES-L2: 添加[[wikilink]]，说明反义关系 %%

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[create]] | cre(增长) + -ate(使) | 创造 | 词义补充（creative 的动词形式） |
> | [[creation]] | create + -ion | 创造，创造物 | 词义补充 |
> | [[creativity]] | creative + -ity | 创造力 | 词义补充 |
> | [[creature]] | cre(增长) + -ature | 生物 | 同词根族 |

%% HERMES-L2: 派生词链接(哲学文档2.5节) %%
%% HERMES-L2: 词义补充: 派生词反哺母词 / 词义填充: 母词填充派生词 %%

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **creative thinking**：创造性思维
> - **creative ideas**：创意想法
> - **creative approach**：创造性方法
> - **creative expression**：创造性表达

%% HERMES-L3: 仅列考研真题高频搭配，不编造 %%
%% HERMES-L3: 每个搭配附带1个真题例句 %%

## 真题/语料关联

> [!example]- 语料 · L3
> - "Sternberg notes that traditional tests best assess analytical and verbal skills but fail to measure creativity and practical knowledge."（Sternberg 指出，传统的测试最恰当地评估了分析能力和语言表达能力，但未能衡量创造力和实践知识。——2007年考研英语阅读Text2）
> - "Curriculums should evolve to focus less on memorizing facts and more on creativity and complex communication."（课程应该发展变化，减少对记忆事实的关注，更多地关注创造力和复杂沟通能力。——2018年考研英语阅读）

%% HERMES-L3: 链接到阅读/翻译笔记中的具体段落 %%

---

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-22 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

%% 复习间隔参考 [[词汇学习哲学#与加工深度等级的对应]] %%

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>

%% HERMES-L4: 仅在用户实际产出后填写 %%

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：cre-/cresc-/cret-（增长，拉丁语 *crescere*）
> - [ ] 同义辨析：creative/imaginative/innovative/original
> - [ ] 反义词群：uncreative/routine
> - [ ] 形近词群：creative/create/creation/creativity/creature

%% HERMES-ACTIVATION: AI 根据用户回复勾选对应 checkbox，/回顾时读取勾选状态决定是否纳入对应网络 %%
