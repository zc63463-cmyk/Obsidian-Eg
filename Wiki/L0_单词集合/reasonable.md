---
title: "reasonable"
tags:
  - 学习/英语/词汇
  - 语义场/理性世界
  - 掌握/L0
aliases: []
date: 2026-04-21
mastery: L0
word_freq: 高频
semantic_field: 理性世界
prototype: 具有理性思考能力的，合乎道理的
extension_dim: 抽象路径
last_review: 2026-04-21
review_count: 0
---

# reasonable

> [!info] 基础信息
> **音标** /ˈriːznəbl/ | **词频** 考研高频 | **语义场** [[理性世界]]

## 核心释义

**adj.** ①==**合理的，有道理的**== ; ②==**公道的，公平的**== ; ③==**（人）通情达理的**== ;

> [!tip] 原型义
> **原型义**：具有理性思考能力的，可以被 reason（理性）所接受的
> **延伸维度**：抽象路径
> **隐喻类型**：无隐喻（字面义即为原型义）

## 词根词缀

[[reason]](理性/思考) + -able(可…的，具有…性质的)

**叙事**：reasonable = reason（理性）+ able（可以的）→ "可以被理性接受的"→ 合理的、公道的、通情达理的。

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**具有理性思考能力的**"为统筹中心，沿抽象路径向外扩展：
>
> 1. **具有理性思考能力的**（本义：可以被理性接受的）
>    - → **合理的/有道理的**：<将"理性可接受"应用于观点、解释、要求>
>      - 激活条件：评价观点、解释、要求是否合理时
>    - → **公道的/公平的**：<将"理性"应用于价格、条件等数值>
>      - 激活条件：评价价格、报酬、条件是否公道时
>    - → **通情达理的**：<将"理性"应用于人的性格特征>
>      - 激活条件：描述人的性格是否讲道理时

### 统筹

- **延伸中心**：经得起理性审视的
- **一句话统筹**：一切"经得起理性检验"的——合理的观点、公道的价格、通情达理的人。

> [!check]- 链路验证
> - [x] **可逆性**：从"公道的价格"能反向联想到"经得起理性检验"
> - [x] **可统筹**：理性可接受→合理→公道→通情达理，统一于"经得起理性审视"
> - [x] **有支撑**：a reasonable price（公道的价格）/ a reasonable person（通情达理的人）

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | adj. | 合理的；公道的；通情达理的 | 核心形容词，reason + -able |
> | adv. reasonably | 合理地；相当地 | adj.→adv. +ly |
> | n. reasonableness | 合理性 | adj.→n. +ness |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**：
> reasonable 就是"可以用 [[reason]]（理性）来衡量的"。一个 reasonable 的价格是经得起理性计算的，一个 reasonable 的人是愿意用道理沟通的。记住：reasonable = 合理的 = 公道的 = 通情达理的。
>
> **词拆分记忆**：
> reason(理由) + able(能够) → 能够给出理由的 → "合理的"。

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[rational]] | 理性的 | 强调逻辑推理的严密性 | 决策、分析、行为 | 中性/褒义 |
> | [[logical]] | 合逻辑的 | 强调符合逻辑规则 | 论证、推理 | 中性 |
> | [[fair]] | 公平的 | 强调不偏不倚 | 待遇、价格、判断 | 褒义 |
> | **reasonable** | 合理的/公道的 | 强调经得起理性审视 | 价格、要求、人 | 褒义/中性 |

>
> **"增"标记**：reasonable **(增-实用性+日常性)** vs rational（rational 更偏学术和逻辑，reasonable 更贴近日常判断）

## 反义词

> [!note]- 反义词 · L2
> - [[unreasonable]]：不合理的
> - [[irrational]]：不理性的
> - [[absurd]]：荒谬的

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[reason]] | 拉丁语 ratio(计算) | 理性；理由；推理 | 词义填充：reasonable 的名词词根 |
> | reasonably | reasonable + -ly | 合理地；相当地 | 词义补充：副词形式 |

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **a reasonable price**：The house was sold at a reasonable price.（这栋房子以公道的价格售出。）
> - **reasonable doubt**：The jury found him not guilty beyond a reasonable doubt.（陪审团认为不存在合理疑点，判他无罪。）
> - **perfectly reasonable**：Her request is perfectly reasonable.（她的要求完全合理。）

## 真题/语料关联

> [!example]- 语料 · L3
> > - "Darwinism seems to offer justification, for if all humans share common origins, it seems reasonable to suppose that cultural diversity could also be traced to more constrained beginnings."（达尔文学说似乎提供了证明，因为如果所有人类拥有共同的起源，那么认为文化多样性也可以追溯到更有限的起点似乎是合理的。）——2012年考研翻译
> - "They could still invalidate Fourth Amendment protections when facing severe, urgent circumstances, and they could take reasonable measures to ensure that phone data are not erased or altered while a warrant is pending."（在严重、紧急的情况下，他们仍然可以使第四修正案的保护失效，并且可以采取合理措施确保在等待搜查令期间手机数据不被删除或篡改。）——考研阅读真题

%% HERMES-L3: 链接到阅读/翻译笔记中的具体段落 %%

---

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-21 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：
> **翻译实践**：

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：reason/ratio（计算/思考）
> - [ ] 同义辨析：reasonable / rational / logical / fair
> - [ ] 反义词群：unreasonable / irrational / absurd
> - [ ] 形近词群：reasonable / reason / season
