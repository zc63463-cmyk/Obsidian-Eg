---
title: "downturn"
tags:
  - 学习/英语/词汇
  - 语义场/资本经济
aliases: []
date: 2026-04-24
word_freq: 超纲词
semantic_field: 资本经济
prototype: 向下转（经济/趋势向坏的方向转变）
extension_dim: 时间路径
phonetic: "/ˈdaʊntɜːn/"
pos: n.
metaphor_type: 方位隐喻
word_root: down
network_activation: [词根, 同义辨析, 反义词群]
last_review: 2026-04-24
review_count: 0
---

# downturn

> [!info] 基础信息
> **音标** /ˈdaʊntɜːn/ | **词级** 超纲词 | **语义场** [[资本经济]]

## 核心释义

**n.** ①==**衰退，下降趋势**== `adj N` `N in N`；②==**低迷时期**== `during the N`；

> [!tip] 原型义
> **原型义**：向下转（经济/趋势向坏的方向转变）
> **延伸维度**：时间路径
> **隐喻类型**：方位隐喻（向下= 变差/衰退）

## 词根词缀

down-(向下) + turn(转向) → 向下转 → 衰退
**叙事**：down（向下）+ turn（转向）→ 向下转 → 衰退。经济 downturn 就是经济向下转——从好变坏，从增长变为衰退。

---

## 词义链路

> [!abstract]- 词义链路法
> 以“**向下转（经济/趋势向坏的方向转变）**”为统筹中心，沿时间路径向外扩展：
>
> 1. **向下转**（方向层面）
>    - → **衰退，下降趋势**：经济/趋势向坏的方向转变
>    - 激活条件：economic downturn, market downturn
>    - → **低迷时期**：不景气的阶段
>    - 激活条件：during the downturn

### 统筹（选择适用的模式）

- **一字一词概括**：downturn 就是往下走——经济往下走叫衰退，趋势往下走叫下降
- **延伸中心**：从向下转延伸到经济衰退

> [!check]- 链路验证
> - [ ] **可逆性**：从引申义能否反向推导回原型义？
> - [ ] **可统筹**：能否用一句话将所有义项串联？
> - [ ] **有支撑**：是否有真题例句或语料支撑该链路？

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 衰退；下降趋势 | down + turn（副词+动词→复合名词） |
>
> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> down（向下）+ turn（转）→ 向下转 → 衰退。upturn 是向上转（好转），downturn 是向下转（衰退）。经济 downturn = 经济衰退。
>
> **词拆分记忆**（辅助）：
> down（向下）+ turn（转向）→ 向下转 → 衰退

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[recession]] | 衰退 | 强调经济正式进入衰退期 | 经济 | 强烈消极 |
> | [[decline]] | 下降，衰退 | 最通用 | 任何事物 | 消极 |
> | [[slump]] | 暴跌，衰退 | 强调突然大幅下降 | 经济、销售 | 强烈消极 |
> | **downturn** | 衰退，下降趋势 | 强调向下转变的趋势（最通用） | 经济、市场 | 消极 |
>
> **“增”标记**：downturn **(增-方向转变的动态感和通用性)** vs recession（基准词：经济衰退）

## 反义词

> [!note]- 反义词
> - [[upturn]]：好转，上升（downturn 的反义：衰退↔好转）
> - [[recovery]]：复苏（downturn 的反义：衰退↔复苏）
> - [[boom]]：繁荣（downturn 的反义：衰退↔繁荣）

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|

---

## 搭配与短语

> [!example]- 搭配
> - **economic downturn**：经济衰退
> - **market downturn**：市场低迷
> - **during the downturn**：在衰退期间

## 真题/语料关联

> [!example]- 语料
> - Many businesses struggled to survive the economic downturn. `[例]`
>   - 中译：经济衰退导致失业率急剧上升。
>   - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/downturn
> - The housing market downturn led to a sharp decline in property prices. `[例]`
>   - 中译：旅游业在经历了一次严重下滑后正在缓慢恢复。
>   - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/downturn
> - Small businesses are particularly vulnerable during a market downturn. `[例]`

## 主动产出

> [!success]- 内化标记
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>
