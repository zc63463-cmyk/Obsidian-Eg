---
title: "aversion"
tags:
  - 学习/英语/词汇
  - 语义场/人本世界
aliases: []
date: 2026-04-24
word_freq: 超纲词
semantic_field: 人本世界
prototype: 转离的动作/状态（身体或心理转离某事物的倾向）
extension_dim: 抽象路径
phonetic: "/əˈvɜːʃn/"
pos: n.
metaphor_type: 方位隐喻
word_root: ver-vert
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-24
review_count: 0
---

# aversion

> [!info] 基础信息
> **音标** /əˈvɜːʃn/ | **词级** 超纲词 | **语义场** [[人本世界]]

## 核心释义

**n.** ①==**厌恶，反感，憎恶**== `have an aversion to n` [正式]；②==**反感的事物**== `N be the aversion of n` [正式]；

> [!tip] 原型义
> **原型义**：转离的动作/状态（身体或心理转离某事物的倾向）
> **延伸维度**：抽象路径
> **隐喻类型**：方位隐喻（"转离"= "厌恶"——空间上的转身离开→心理上的排斥厌恶）

## 词根词缀

a-(=ab-, 离开/远离) + [[ver-vert|vers]](转) + -ion(名词后缀，表示动作或状态)

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**转离的动作/状态**"为统筹中心，沿抽象路径向外扩展：
>
> 1. **转离的动作/状态**（物理层面）
>    - → **厌恶，反感**：从身体转离扩展到心理上的排斥和厌恶
>      - 激活条件：have an aversion to, aversion to risk
>    - → **反感的事物**：从厌恶的情感扩展到被厌恶的对象
>      - 激活条件：His aversion to crowds, be the aversion of

### 统筹（选择适用的模式）

- **一字一词概括**：aversion 就是"转离"——身体转离某物叫"回避"，心理转离某事叫"厌恶"
- **延伸中心**：从"身体转离"延伸到"心理厌恶"再到"被厌恶的事物"

> [!check]- 链路验证
> - [x] **可逆性**：从"厌恶"能反向推导到"转离"——厌恶就是心理上"转过去不想面对"
> - [x] **可统筹**：一句话——"aversion 就是转离某物的心理倾向，即厌恶"
> - [x] **有支撑**：have an aversion to 为固定搭配，广泛见于学术和正式文体

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 厌恶，反感；反感的事物 | 核心词性，[[averse]] 的名词形式（转离的状态→厌恶） |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> aversion 是 [[averse]] 的名词形式。a-（远离）+ vers（转）+ -ion（名词后缀）→ "转离的状态"→ 厌恶。你看到蟑螂，身体本能地"转开"——这种"转离"的倾向就是 aversion。have an aversion to sth. 就是"对某物有转离的倾向"→ 对某物有厌恶。和 [[averse]] 的关系：averse 是"反对的"（形容词），aversion 是"厌恶"（名词）。
>
> **词拆分记忆**（辅助）：
> a（离开）+ vers（转）+ ion（名词化）→ 转离的状态 → 厌恶

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[disgust]] | 厌恶，恶心 | 强调生理上的恶心感 | 食物、行为 | 强烈/生理性 |
> | [[repugnance]] | 极度反感 | 强调道德或理性上的排斥 | 行为、观念 | 强烈/正式 |
> | [[dislike]] | 不喜欢 | 强调一般性的不喜欢 | 人、事物 | 弱/日常 |
> | **aversion** | 厌恶，反感 | 强调本能的回避和排斥倾向 | 风险、行为、事物 | 中等/正式 |
>
> **"增"标记**：aversion **(增-回避倾向和本能性)** vs dislike（基准词：不喜欢）；aversion **(增-正式书面语体和持续性)** vs disgust（基准词：恶心）

## 反义词

> [!note]- 反义词
> - [[affinity]]：喜爱，亲和力（aversion 的反义：厌恶 ↔ 喜爱）
> - [[penchant]]：爱好，嗜好（aversion 的反义：反感 ↔ 爱好）
> - [[inclination]]：倾向，意愿（aversion 的反义：排斥倾向 ↔ 倾向）

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[averse]] | a-(远离) + vers(转) + -e | 反对的，不乐意的 | 词义补充（aversion 的形容词形式） |
> | [[aversive]] | a-(远离) + vers(转) + -ive(倾向) | 令人厌恶的 | 词义补充 |

> [!note] 补充说明
> averse（形容词）描述人的态度"反对的"，aversion（名词）是这种态度本身"厌恶"，aversive（形容词）描述引起厌恶的事物"令人厌恶的"。三者构成完整的语义链条。

---

## 搭配与短语

> [!example]- 搭配
> - **have an aversion to sth.**：对某事有厌恶/反感
> - **aversion to risk/changes**：对风险/变革的厌恶
> - **develop an aversion to**：逐渐对……产生厌恶
> - **an aversion therapy**：厌恶疗法（心理学）
> - **risk aversion**：风险厌恶（经济学/金融学）

## 真题/语料关联

> [!example]- 语料
> - She has a deep aversion to any form of dishonesty. `[例]`
>   - 中译：她对任何形式的不诚实都深感厌恶。
>   - 来源：Oxford Learner's Dictionaries | https://www.oxfordlearnersdictionaries.com/definition/english/aversion
> - Risk aversion among investors has led to a flight to safer assets. `[例]`
>   - 中译：投资者的风险厌恶情绪导致了资金向更安全的资产转移。
>   - 来源：Cambridge Dictionary | https://dictionary.cambridge.org/dictionary/english/aversion
> - His aversion to public speaking held back his career for years. `[例]`
>   - 中译：他对公开演讲的厌恶多年来阻碍了他的职业发展。
>   - 来源：Collins Dictionary | https://www.collinsdictionary.com/dictionary/english/aversion

## 主动产出

> [!success]- 内化标记
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>
