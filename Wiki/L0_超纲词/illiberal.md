---
title: "illiberal"
tags:
  - 学习/英语/词汇
  - 语义场/社会习俗
aliases: []
date: 2026-04-24
word_freq: 超纲词
semantic_field: 社会习俗
prototype: 缺乏自由和宽容的态度
extension_dim: 社会路径
phonetic: "/ɪˈlɪbərəl/"
pos: adj.
metaphor_type: 结构隐喻
word_root: liber
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-24
review_count: 0
---

# illiberal

> [!info] 基础信息
> **音标** /ɪˈlɪbərəl/ | **词级** 超纲词 | **语义场** [[社会习俗]]

## 核心释义

**adj.** ①==**狭隘的，偏执的**== `illiberal attitudes` [书面] `illiberal views` [书面]；②缺乏宽容的；③反对自由的（政治上） `illiberal regime` [书面]；

> [!tip] 原型义
> **原型义**：缺乏自由和宽容的态度
> **延伸维度**：社会路径
> **隐喻类型**：结构隐喻（自由=开放空间，不自由=封闭空间）

## 词根词缀

il-(=in-, 不) + [[liber|liber]](自由) + -al(形容词后缀)
**叙事**：il（不）+ liber（自由）+ al（的）→ 不自由的 → 狭隘的、偏执的。一个 illiberal 的人，心胸像被锁住一样打不开。

---

## 词义链路

> [!abstract]- 词义链路法
> 以“**缺乏自由和宽容的态度**”为统筹中心，沿社会路径向外扩展：
>
1. **缺乏自由和宽容**（态度层面）
   - → **狭隘的，偏执的**：思想不开放
     - 激活条件：illiberal attitudes, illiberal views
   - → **反对自由的**：政治立场
     - 激活条件：illiberal regime, illiberal policies

### 统筹（选择适用的模式）

- **一句话统筹**：illiberal 就是“不自由”——思想上叫狭隘偏执，政治上叫反对自由

> [!check]- 链路验证
> - [ ] **可逆性**：从引申义能否反向推导回原型义？
> - [ ] **可统筹**：能否用一句话将所有义项串联？
> - [ ] **有支撑**：是否有真题例句或语料支撑该链路？

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
|------|------|----------|
| adj. | 狭隘的；缺乏宽容的；反对自由的 | il-(不) + liber(自由) + -al(的) |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> il（不）+ liber（自由）→ 不自由的、狭隘的。liberal 是宽容开放的，illiberal 就是不宽容、心胸狭隘的。

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
|------|------------|---------|---------|---------|
| [[intolerant]] | 不宽容的 | 强调无法接受不同意见 | 态度、信仰 | 贬义 |
| [[narrow-minded]] | 心胸狭隘的 | 强调视野受限 | 思想、观点 | 贬义 |
| [[bigoted]] | 固执偏见的 | 强调根深蒂固的偏见 | 种族、宗教 | 强烈贬义 |
| **illiberal** | 狭隘的；反对自由的 | 强调缺乏自由宽容精神 | 态度、政策 | 贬义/正式 |

**“增”标记**：illiberal **(增-政治哲学维度的深度)** vs intolerant（基准词：不宽容）

## 反义词

> [!note]- 反义词
> - [[liberal]]：自由的，宽容的（illiberal 的精确反义：不自由↔自由）
- [[tolerant]]：宽容的

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
|--------|---------|------|---------|
| [[liberal]] | liber(自由) + -al | 自由的；开明的 | 词义补充 |
| [[liberalism]] | liberal + -ism | 自由主义 | 词义补充 |
| [[illiberalism]] | illiberal + -ism | 反自由主义 | 词义补充 |

---

## 搭配与短语

> [!example]- 搭配
> - **illiberal attitudes**：狭隘的态度
- **illiberal views**：偏执的观点
- **illiberal regime**：不自由的政权

## 真题/语料关联

> [!example]- 语料
> - The government's illiberal policies have drawn criticism from international human rights organizations. `[例]`
>   - 中译：该政府的不开明政策受到了国际人权组织的批评。
>   - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/illiberal
> - His illiberal attitudes toward immigration alienated many of his colleagues. `[例]`
>   - 中译：他对移民的不开明态度疏远了许多同事。
>   - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/illiberal
> - Scholars warn that the rise of illiberal democracy threatens the foundations of constitutional governance. `[例]`
>   - 中译：学者们警告说，不自由民主的崛起威胁着宪政治理的根基。
>   - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/illiberal

## 主动产出

> [!success]- 内化标记
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>
