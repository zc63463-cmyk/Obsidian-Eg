---
title: "roast"
tags:
  - 学习/英语/词汇
  - 语义场/社会习俗
aliases: []
date: 2026-04-24
word_freq: 基础词
semantic_field: 社会习俗
prototype: 烤
extension_dim: [具身路径]
phonetic: "/roʊst/"
pos: v.
metaphor_type: 本体隐喻
word_root: roast
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-24
review_count: 0
---

# roast

> [!info] 基础信息
> **音标** /roʊst/ | **词级** 基础词 | **语义场** [[社会习俗]]

## 核心释义

**v.** ①==**烤，烘**== `roast chicken; roast beef` [中性]；②==**严厉批评，嘲讽**== `roast the politician; get roasted` [口语/贬义]；
**n.** ①==**烤肉**== `a traditional Sunday roast` [中性]；②==**嘲讽（节目）**== `a roast comedy` [口语/中性]；
**adj.** ①==**烤好的**== `roast duck; roast potatoes` [中性]；

> [!tip] 原型义
> **原型义**：烤（to cook with dry heat）
> **延伸维度**：具身路径
> **隐喻类型**：本体隐喻（将"火烤"隐喻为"言语上的严厉批评"）

## 词根词缀

roast（烤，古法语 *rostir*，可能来自日耳曼语源，与 *rust* "红色"有关——烤到变红）
**叙事**：roast 就是最基本的"烤"——把肉放在火上烤。后来引申为用言语"烤"人——严厉批评。

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**烤**"为统筹中心，沿具身路径向外扩展：
>
> 1. **烤**（烹饪层面）
>    - → **烤，烘**：用干热烹饪食物
>      - 激活条件：roast chicken, roast beef
>    - → **严厉批评，嘲讽**：用言语"烤"人
>      - 激活条件：roast the politician, get roasted
>    - → **烤肉**（名词）：烤出来的食物
>      - 激活条件：Sunday roast

### 统筹（选择适用的模式）

- **一字一词概括**：roast 就是"烤"——烤肉、烤人（批评）
- **延伸中心**：从"火烤食物"延伸到"言语烤人"

> [!check]- 链路验证
> - [x] **可逆性**：从"严厉批评"能反向推导到"烤"——批评就像用火烤人一样难受
> - [x] **可统筹**：一句话——"roast 就是用火烤（食物或人）"
> - [x] **有支撑**：roast chicken, get roasted 均有语料支撑

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | v. | 烤；严厉批评 | 核心词性 |
> | n. | 烤肉；嘲讽 | v→n：烤的结果/行为 |
> | adj. | 烤好的 | v→adj：+ed/直接转化 |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> 古法语 rostir，可能和"红色"有关——烤到变红。roast chicken（烤鸡）是西餐经典。在口语中，roast someone 就是"把某人放在火上烤"——严厉批评或嘲讽。美国的 roast comedy（吐槽大会）就是专门"烤"名人的节目。
>
> **词拆分记忆**（辅助）：
> roa（road 路）+ st（street 街）→ 路边摊烤肉 → 烤

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[bake]] | 烘焙 | 强调用烤箱/干热 | 面包、蛋糕 | 中性 |
> | [[grill]] | 烧烤 | 强调直接接触火焰 | 肉、蔬菜 | 中性 |
> | **roast** | 烤；严厉批评 | 强调整体干热烹饪 | 肉、人 | 中性/可贬义 |
>
> **"增"标记**：roast **(增-整体性和大块性)** vs grill（基准词：直接烧烤）；roast **(增-批评引申义)** vs bake（基准词：一般烘焙）

## 反义词

> [!note]- 反义词
> - [[boil]]：煮（roast 的反义：烤↔煮）
> - [[praise]]：赞扬（roast 的反义：严厉批评↔赞扬）

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[roaster]] | roast + -er(人/物) | 烤箱；烤的人 | 词义补充 |

---

## 搭配与短语

> [!example]- 搭配
> - **roast chicken/beef**：烤鸡/烤牛肉
> - **roast someone**：严厉批评某人
> - **fit to roast**：适合烤的

## 真题/语料关联

> [!example]- 语料
> - Roast lamb with all the trimmings is my favourite meal. `[真题]`
>   - 中译：【需人工审核】roast 的例句。
>   - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/roast
  - 中译：配上各种配菜的烤羊排是我最爱吃的菜。
  - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/roast
> - The restaurant is famous for its roast duck. `[真题]`
  - 中译：这家餐厅以烤鸭闻名。
  - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/roast
> - The comedian roasted the audience with sharp wit. `[真题]`
  - 中译：喜剧演员以犀利的机智调侃了观众。
  - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/roast


---

## 主动产出

> [!success]- 主动产出
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>
