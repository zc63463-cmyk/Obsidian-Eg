---
title: "versus"
tags:
  - 学习/英语/词汇
  - 语义场/抽象关系
  - 掌握/L0
aliases: []
date: 2026-04-23
mastery: L0
word_freq: 高频
semantic_field: 抽象关系
prototype: 对，与……相对（两个方向转向对立）
extension_dim: 抽象路径
last_review: 2026-04-23
review_count: 0
---

# versus

> [!info] 基础信息
> **音标** /ˈvɜːsəs/ | **词频** 考研高频 | **语义场** [[抽象关系]]

## 核心释义

**prep.** ①==**与……相对，与……相比**==；②（比赛等）对；（法律）诉；

> [!tip] 原型义
> **原型义**：对，与……相对（两个方向转向对立）
> **延伸维度**：抽象路径
> **隐喻类型**：方位隐喻（"转向对立"→ 比赛/对比/诉讼中的对立关系）

## 词根词缀

[[ver-vert|vers]](=turn toward，转向，拉丁语 *vertere*，与 [[ver-vert]]/[[versatile]] 同源) + -us(拉丁语介词后缀)
**叙事**：vers（转向）+ -us → 转向对立面 → 与……相对。versus 的本义就是"转向"——两个队伍在比赛中"转向"彼此，形成对立。France versus Brazil 就是法国队"转向"巴西队（法国对巴西）。在法律中，Brown versus Board of Education 就是布朗"转向"教育委员会（布朗诉教育委员会案）。

已有词根笔记 [[ver-vert]]

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**对，与……相对（两个方向转向对立）**"为统筹中心，沿抽象路径向外扩展：
>
> 1. **与……相对**（关系层面）
>    - → **比赛中的"对"**：两个队伍转向对立
>      - 激活条件：France versus Brazil, Harvard versus Yale
>    - → **对比中的"与……相比"**：两个事物转向对立
>      - 激活条件：cost versus benefit, quality versus quantity
>    - → **法律中的"诉"**：两个 parties 转向对立
>      - 激活条件：Brown versus Board of Education

### 统筹（选择适用的模式）

- **延伸中心**：versus 的核心是"两个方向的对立"——比赛中是对手，对比中是对照，法律中是原告被告
- **一句话统筹**：versus 就是"两个方向转向对立"——比赛、对比、诉讼中的"对/诉/比"

> [!check]- 链路验证
> - [x] **可逆性**：从"诉"能反向推导到"转向对立"——诉讼就是两个方向转向对立的法律形式
> - [x] **可统筹**：一句话——"versus 就是两个方向的对立，从比赛到对比到诉讼"
> - [x] **有支撑**：France versus Brazil, cost versus benefit 均有语料支撑

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | prep. | 与……相对；对；诉 | 核心词性，唯一的词性 |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> versus 来自 [[ver-vert]]（转）词根。vers（转向）→ versus 就是两个方向"转向"彼此，形成对立。最常见的缩写是 vs.——NBA 总决赛 Lakers vs. Celtics，就是湖人队"转向"凯尔特人队（湖人对凯尔特人）。在学术论文中，cost versus benefit（成本与收益对比），quality versus quantity（质量与数量对比）。在法律中，Brown versus Board of Education 就是著名的"布朗诉教育委员会案"。
>
> **词拆分记忆**（辅助）：
> vers（转向）+ us（我们）→ 我们转向彼此 → 对立 → 对

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[against]] | 反对，对着 | 最通用，不强调正式性 | 任何对立 | 中性/通用 |
> | [[compared with]] | 与……相比 | 强调比较关系 | 数据、观点 | 中性/比较 |
> | **versus** | 对；与……相比；诉 | 强调正式的对立关系（最正式） | 比赛、法律、学术 | 正式/对立 |
>
> **"增"标记**：versus **(增-正式性和法律含义)** vs against（基准词：一般对立）；versus **(增-缩写 vs. 的通用性)** vs compared with（基准词：一般比较）

## 反义词

> [!note]- 反义词 · L2
> - [[with]]：与……一起（versus 的反义：对立↔合作）

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[veil]] | 与 versus 同源（覆盖） | 面纱，遮掩 | 词族关联 |
> | [[vein]] | 与 versus 同源（血管） | 静脉；风格 | 词族关联 |
> | [[velocity]] | 与 versus 同源（veloc-快速） | 速度 | 词族关联 |
> | [[velvet]] | 与 versus 同源（覆盖） | 天鹅绒 | 词族关联 |
> | [[vertical]] | 与 versus 同源（转动方向） | 垂直的 | 词族关联 |
> | [[verge]] | 与 versus 同源（边缘） | 边缘，濒临 | 词族关联 |
> | [[versatile]] | 与 versus 同源（转动） | 多才多艺的 | 词族关联 |

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **A versus B**：A 对 B / A 与 B 相比
> - **cost versus benefit**：成本与收益对比
> - **quality versus quantity**：质量与数量对比
> - **man versus machine**：人与机器
> - **缩写 vs.**：France vs. Brazil

## 真题/语料关联

> [!example]- 语料 · L3
> - "The concept of man versus machine is at least as old as the industrial revolution."（人与机器对立的概念至少可以追溯到工业革命时期。——2014年考研英语二阅读Text 3）
> - "The most exciting game was Harvard versus Yale."（最富紧张刺激的球赛是哈佛队对耶鲁队。——柯林斯语料库）
> - "Only 18.8% of the class of 1982 had some kind of diploma four years after high school, versus 45% of the class of 1972."（1982届高中毕业生中只有18.8%在四年后获得了某种文凭，相比之下1972届为45%。——柯林斯语料库）

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-23 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：[[ver-vert|ver-/vert-]]（转，拉丁语 *vertere*）
> - [ ] 同义辨析：versus/against/compared with
> - [ ] 反义词群：with
> - [ ] 形近词群：versus/vs./versatile
