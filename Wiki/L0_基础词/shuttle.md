---
title: "shuttle"
tags:
  - 学习/英语/词汇
  - 语义场/人本世界
aliases: []
date: 2026-04-22
word_freq: 基础词
semantic_field: 人本世界
prototype: 梭子（织布时来回穿行的工具）
extension_dim: [空间路径]
phonetic: "/ˈʃʌtl/"
pos: n.
metaphor_type: 方位隐喻
word_root: bus
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-22
review_count: 0
---

# shuttle

> [!info] 基础信息
> **音标** /ˈʃʌtl/ | **词级** 基础词 | **语义场** [[人本世界]]

## 核心释义

**n.** ①==**穿梭班机/列车/汽车**== `shuttle bus/service; airport shuttle` [中性]；②航天飞机；③梭子（织布用）；
**v.** ①==**穿梭往返**== `shuttle between A and B` [中性]；②频繁地往返于两地之间；

> [!tip] 原型义
> **原型义**：梭子（织布时在经线之间来回穿行的工具）
> **延伸维度**：空间路径
> **隐喻类型**：运动隐喻（"梭子来回穿行"= 在两点之间频繁往返→穿梭班车→航天飞机）

## 词根词缀

本词为日耳曼语源（古英语 *scytel*，意为"飞镖、梭子"，与 [[shoot]]"射出"同源），-le 为指小后缀。
**叙事**：shuttle 本义是织布的梭子——在经线之间来回穿行。延伸到交通——在两点之间来回的班车——a shuttle bus（穿梭巴士）。延伸到太空——在地球和太空之间来回的飞行器——space shuttle（航天飞机）。shuttle 的核心是"来回穿行"。

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**梭子（来回穿行）**"为统筹中心，沿空间路径向外扩展：
>
> 1. **梭子**（物体层面）
>    - → **梭子**：织布工具
>      - 激活条件：a weaver's shuttle
>    - → **穿梭班机/列车**：在两点之间来回
>      - 激活条件：shuttle bus, shuttle service
>    - → **航天飞机**：地球和太空之间来回
>      - 激活条件：space shuttle
>    - → **穿梭往返**（动词）：频繁往返
>      - 激活条件：shuttle between A and B

### 统筹（选择适用的模式）

- **一字一词概括**：shuttle 就是"来回"——梭子来回织布，班车来回接送
- **延伸中心**：从"梭子"延伸到"穿梭班车"和"航天飞机"

> [!check]- 链路验证
> - [x] **可逆性**：从"穿梭班车"能反向推导到"梭子"——班车就像梭子一样来回
> - [x] **可统筹**：一句话——"shuttle 就是来回穿行，从梭子到班车到航天飞机"
> - [x] **有支撑**：shuttle bus, space shuttle, shuttle between 均有语料支撑

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 穿梭班车；航天飞机；梭子 | 核心词性 |
> | v. | 穿梭往返 | 由名词"梭子"引申为动词"来回穿行" |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> shuttle 本义是织布的梭子——在经线之间来回穿行。机场到市区的穿梭巴士——a shuttle bus。地球和太空之间来回的航天飞机——space shuttle。外交官在两个首都之间来回穿梭——shuttle between the two capitals。shuttle 就是"来回穿行"。
>
> **词拆分记忆**（辅助）：
> shut（关闭）+ tle → 梭子来回穿行像关门又开门 → shuttle 穿梭

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[commute]] | 通勤，往返 | 强调日常上下班的往返 | 上班族 | 中性 |
> | [[ferry]] | 渡船；摆渡 | 强调水上的往返运输 | 渡船、乘客 | 中性 |
> | **shuttle** | 穿梭班车；航天飞机 | 强调两点间频繁往返（最形象） | 班车、飞机、人 | 中性 |

> **"增"标记**：shuttle **(增-航天飞机的太空义)** vs commute（基准词：一般通勤）；shuttle **(增-梭子的本义)** vs ferry（基准词：水上运输）

## 反义词

> [!note]- 反义词
> - [[stay]]：停留（shuttle 的反义：穿梭往返↔停留不动）

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[sheer]] | — | 纯粹的；陡峭的；极薄的 | 同族词 |
> | [[sheet]] | — | 薄片；床单；纸张 | 同族词 |
> | [[shepherd]] | — | 牧羊人；带领 | 同族词 |
> | [[shrewd]] | — | 精明的，敏锐的 | 同族词 |
> | [[shrink]] | — | 收缩；退缩 | 同族词 |
> | [[shut]] | — | 关闭 | 同族词 |
> | [[shutter]] | — | 百叶窗；快门 | 同族词 |
> | [[shy]] | — | 害羞的；投掷 | 同族词 |

---

## 搭配与短语

> [!example]- 搭配
> - **shuttle bus/service**：穿梭巴士/服务
> - **space shuttle**：航天飞机
> - **shuttle between A and B**：在A和B之间穿梭
> - **a shuttle diplomacy**：穿梭外交
> - **airport shuttle**：机场穿梭巴士

## 真题/语料关联

> [!example]- 语料
> - All the meals on space station are put together on Earth and sent there by space shuttle. `[真题]`
>   - 中译：【需人工审核】shuttle 的例句。
>   - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/shuttle
  - 中译：空间站上的所有餐食都是在地球上准备好，由航天飞机运送过去的。；
  - 来源：Oxford Learner's Dictionaries | https://www.oxfordlearnersdictionaries.com/definition/english/shuttle；
> - During the next two years, other delegations shuttled between the two capitals. `[例]`
  - 中译：在接下来的两年里，其他代表团在两个首都之间来回穿梭。；
  - 来源：Oxford Learner's Dictionaries | https://www.oxfordlearnersdictionaries.com/definition/english/shuttle；
> - The hotel provides a shuttle bus that takes you to and from the airport. `[例]`
  - 中译：酒店提供往返机场的穿梭巴士。；
  - 来源：Cambridge Dictionary | https://dictionary.cambridge.org/dictionary/english/shuttle；


---

## 主动产出

> [!success]- 主动产出
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>
