---
title: "resign"
tags:
  - 学习/英语/词汇
  - 语义场/社会专业
  - 掌握/L0
aliases: []
date: 2026-04-23
mastery: L0
word_freq: 高频
semantic_field: 社会专业
prototype: 重新签名，放弃
extension_dim: 辞职顺从
last_review: 2026-04-23
review_count: 0
---

# resign

> [!info] 基础信息
> **音标** /rɪˈzaɪn/ | **词频** 考研高频 | **语义场** [[社会专业]]

## 核心释义

**v.** ①==**辞职，辞去**==；②==**顺从，听任**==；③==**委托，交托**==（古义）；

> [!tip] 原型义
> **原型义**：重新签名，放弃（re- + sign，"再次签名"→ 签字放弃→ 辞职）
> **延伸维度**：辞职顺从
> **隐喻类型**：签名行为隐喻（"签字放弃"→ "辞职"→ "顺从"——从签字行为到放弃权利）

## 词根词缀

re-(前缀：再/回) + sign(签名，[[sign]])
**叙事**：resign 来自拉丁语 *resignare*，re- = 再/回，sign = 签名（[[sign]]）。核心逻辑是"再次签名"→ "签字放弃"→ "辞职"。resign from one's position = 辞去职位。resign oneself to = 听任，顺从于。resign 的过去式是 resigned，过去分词也是 resigned。注意 resign 的发音是 /rɪˈzaɪn/，而 [[resignation]]（辞职）的发音是 /ˌrezɪɡˈneɪʃn/（g 发音）。与 [[sign]]（签名）、[[assign]]（分配）、[[design]]（设计）同源。

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**签字放弃/辞职**"为统筹中心，沿辞职顺从向外扩展：
>
> 1. **签字放弃**（行为层面）
>    - → **辞职，辞去**：签字放弃职位
>      - 激活条件：resign from, resign one's post
>    - → **顺从，听任**：放弃抵抗
>      - 激活条件：resign oneself to fate
>    - → **委托，交托**：签字移交（古义）
>      - 激活条件：resign something to someone

### 统筹（选择适用的模式）

- **一字一词概括**：resign 就是"签字放弃"——从辞职到顺从到委托
- **延伸中心**：从"放弃职位"延伸到"放弃抵抗"

> [!check]- 链路验证
> - [x] **可逆性**：从"顺从"能反向推导到"签字放弃"——顺从就是放弃抵抗
> - [x] **可统筹**：一句话——"resign = 签字放弃→辞职→顺从"
> - [x] **有支撑**：resign from, resign oneself to 均有语料支撑

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | v. | 辞职；顺从；委托 | 核心词性 |
> | n. | （用 [[resignation]] 表示） | v.→n：用派生名词 |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> resign = re-（再）+ sign（签名，[[sign]]）= "再次签名"→ 签字放弃→ 辞职。想象一个员工在辞职信上签字——那就是 resign。resign from the company = 从公司辞职。resign one's post = 辞去职务。resign oneself to fate = 听天由命（放弃抵抗命运）。注意 resign 有两个重要含义：1）辞职（主动放弃）；2）顺从（被动接受）。与 [[sign]]（签名）、[[assign]]（分配）、[[design]]（设计）同源——都和"签名/标记"有关。
>
> **词拆分记忆**（辅助）：
> re（再）+ sign（签名）→ 再次签名（放弃权利）→ 辞职 → resign

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[quit]] | 辞职，放弃 | 最口语化的辞职表达 | 工作、习惯 | 中性/口语 |
> | [[step down]] | 退位，辞职 | 强调主动让位 | 职位、权力 | 中性/委婉 |
> | [[retire]] | 退休 | 强调因年龄退出 | 工作 | 正面 |
> | **resign** | 辞职；顺从 | 最正式的辞职表达 | 职位、权力 | 正式/中性 |

> **"增"标记**：resign **(增-最正式的辞职表达)** vs quit（基准词：辞职）；resign **(增-兼具"顺从"的含义)** vs step down（基准词：退位）

## 反义词

> [!note]- 反义词 · L2
> - [[assume]]：就任（resign 辞职的反义：辞职↔就任）
> - [[resist]]：抵抗（resign 顺从的反义：顺从↔抵抗）
> - [[protest]]：抗议（resign 听任的反义：听任↔抗议）

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[resign]] | re- + sign | 辞职；顺从 | 核心词 |
> | [[resignation]] | resign + -ation | 辞职；辞职信；顺从 | 派生名词 |
> | [[resigned]] | resign + -ed | 辞职的；顺从的 | 派生形容词 |
> | [[sign]] | 拉丁语 *signum* | 签名；标记 | 同源词 |
> | [[assign]] | as- + sign | 分配；指派 | 同源词 |
> | [[design]] | de- + sign | 设计；计划 | 同源词 |

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **resign from**：辞去……的职务
> - **resign one's post/position**：辞去职务
> - **resign oneself to**：听任，顺从于
> - **resign in protest**：抗议辞职
> - **force someone to resign**：迫使某人辞职

## 真题/语料关联

> [!example]- 语料 · L3
> - "The CEO resigned from his position following allegations of financial misconduct."（首席执行官在面临财务不当行为指控后辞去了职务。——考研英语阅读商业类话题，resign from 高频考点）
> - "Faced with overwhelming evidence, the defendant resigned himself to the inevitable outcome of the trial."（面对压倒性的证据，被告只能听任审判的必然结果。——考研英语阅读法律类话题，resign oneself to 高频表达）

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-23 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 同义辨析：resign/quit/step down/retire
> - [ ] 反义词群：assume/resist/protest
> - [ ] 同源词群：resign/resignation/sign/assign/design
> - [ ] 词根笔记：sign（标记/签名）
