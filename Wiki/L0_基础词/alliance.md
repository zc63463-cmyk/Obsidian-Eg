---
title: "alliance"
tags:
  - 学习/英语/词汇
  - 语义场/政治法律
  - 掌握/L0
aliases: []
date: 2026-04-23
mastery: L0
word_freq: 中频
semantic_field: 政治法律
prototype: 绑在一起（多方联合）
extension_dim: 社会路径
last_review: 2026-04-23
review_count: 0
---

# alliance

> [!info] 基础信息
> **音标** /əˈlaɪəns/ | **词频** 考研中频 | **语义场** [[政治法律]]

## 核心释义

**n.** ①==**联盟，同盟**==；②==**结盟，联姻**==；

> [!tip] 原型义
> **原型义**：绑在一起（多方联合为一个整体）
> **延伸维度**：社会路径
> **隐喻类型**：结构隐喻（"物理绑定"→"政治/社会联合"）

## 词根词缀

all（全部）+ -iance（名词后缀，表示状态）
**叙事**：all（全部）+ iance（状态）→ 全部绑在一起的状态 → 联盟。alliance 的核心就是"全部在一起"——多个国家/政党绑在一起就是 alliance。与 [[allegiance]]（忠诚）同源——allegiance 是个人绑向组织，alliance 是组织之间互相绑。

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**绑在一起**"为统筹中心，沿社会路径向外扩展：
>
> 1. **绑在一起**（物理层面）
>    - → **联盟，同盟**：国家/政党之间的联合
>      - 激活条件：in alliance with, form an alliance
>    - → **联姻**：家族之间的"绑定"
>      - 激活条件：by alliance

### 统筹（选择适用的模式）

- **一字一词概括**：alliance 就是"绑"——国家绑在一起是联盟，家族绑在一起是联姻
- **延伸中心**：从"多方绑定"延伸到"政治联盟"和"家族联姻"

> [!check]- 链路验证
> - [x] **可逆性**：从"联盟"能反向推导到"绑在一起"——联盟就是国家之间绑在一起
> - [x] **可统筹**：一句话——"alliance 就是'全部绑在一起'的联合状态"
> - [x] **有支撑**：in alliance with, its alliance with 均有真题语料支撑

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 联盟，同盟；结盟，联姻 | 核心词性 |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> all（全部）+ iance（状态）→ 全部在一起 → 联盟。二战时的 Allies（同盟国）就是 all（全部）绑在一起对抗敌人。alliance 和 [[allegiance]] 只差一个字母——allegiance 是你向组织"绑"（忠诚），alliance 是组织之间互相"绑"（联盟）。
>
> **词拆分记忆**（辅助）：
> all（全部）+ i + ance → 全部站在一起 → 联盟

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[coalition]] | 联合，联盟 | 强调不同团体为共同目标联合 | 政党、组织 | 正式/政治 |
> | [[union]] | 联合，工会 | 强调合为一体 | 工人、国家 | 正面/团结 |
> | [[league]] | 联盟，联赛 | 强调有组织的联合 | 国家、运动队 | 中性/有组织 |
> | **alliance** | 联盟，同盟 | 强调"绑在一起"的联合关系 | 国家、政党、组织 | 中性/通用 |

> **"增"标记**：alliance **(增-绑定关系的强调)** vs coalition（基准词：目标联合）；alliance **(增-通用性)** vs league（基准词：有组织）

## 反义词

> [!note]- 反义词 · L2
> - [[separation]]：分离（alliance 的反义：联合↔分离）
> - [[division]]：分裂

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[all]] | 词族母词 | 全部 | 词族互链 |
> | [[allege]] | al- + leg(说) | 宣称 | 词族互链 |
> | [[allegiance]] | al- + lig(绑) + -ance | 忠诚 | 词族互链 |
> | [[alleviate]] | al- + lev(轻) + -iate | 减轻 | 词族互链 |
> | [[allowance]] | allow + -ance | 津贴 | 词族互链 |
> | [[alloy]] | al- + loy(=lig, 绑) | 合金 | 词族互链 |
> | [[ally]] | al- + ly(=lig, 绑) | 同盟者 | 词义补充（alliance 的成员） |
> | [[alone]] | all + one | 独自的 | 词族互链 |
> | [[along]] | all + -ong(长) | 沿着 | 词族互链 |
> | [[alongside]] | along + side | 在……旁边 | 词族互链 |

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **in alliance with**：与……结盟
> - **form/enter into an alliance**：结成联盟
> - **its alliance with**：它与……的联盟

## 真题/语料关联

> [!example]- 语料 · L3
> - "In February the FTC and the Digital Advertising Alliance (DAA) agreed that the industry would get cracking on responding to DNT requests."（二月份，联邦贸易委员会和数字广告联盟达成共识：该行业将尽快响应"禁止追踪"的请求——2013年英语一Text 2）
> - "A native literary drama had been created, its alliance with the public playhouses established, and at least some of its great traditions had been begun."（一种本土戏剧已经诞生，它与公共剧院的联盟已经建立，至少它的一些伟大传统已经开始——2018年英语一翻译）

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-23 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词族网络：all/allege/allegiance/alleviate/alliance/allowance/alloy/ally/alone/along/alongside
> - [ ] 同义辨析：alliance/coalition/union/league
> - [ ] 反义词群：separation/division
