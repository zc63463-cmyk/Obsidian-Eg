---
title: "medieval"
tags:
  - 学习/英语/词汇
  - 语义场/社会习俗
  - 掌握/L0
aliases: []
date: 2026-04-23
mastery: L0
word_freq: 中频
semantic_field: 社会习俗
prototype: 中间的时代的（中世纪的）
extension_dim: 时间路径
last_review: 2026-04-23
review_count: 0
---

# medieval

> [!info] 基础信息
> **音标** /ˌmediˈiːvl/ | **词频** 考研中频 | **语义场** [[社会习俗]]

## 核心释义

**adj.** ①==**中世纪的（约公元5-15世纪）**==；②老式的，过时的（贬义）；

> [!tip] 原型义
> **原型义**：中间的时代的（介于古代和现代之间的时期）
> **延伸维度**：时间路径
> **隐喻类型**：无隐喻（直接指称历史时期）

## 词根词缀

med-(=medius, 中间的，拉丁语 *medius*) + ev(=aevum, 时代/年龄，拉丁语 *aevum*) + -ial(形容词后缀)
**叙事**：med（中间）+ ev（时代）+ ial（的）→ **中间时代的** → 中世纪的。medieval 指的是欧洲历史上介于古典时代（古希腊罗马）和现代之间的那段"中间"时期。

建议创建词根笔记 [[med-medius-]]（中间，拉丁语 *medius*）

---

## 词义链路

> [!tip]- 词义链路法 · L1
> 以"**中间的时代的**"为统筹中心，沿时间路径向外扩展：
>
> 1. **中间的时代的**（时间层面）
>    - → **中世纪的**：欧洲历史上约5-15世纪的时期
>      - 激活条件：medieval period, medieval Europe, medieval architecture
>    - → **老式的，过时的**：像中世纪一样落后的
>      - 激活条件：medieval attitudes, medieval methods

### 统筹（选择适用的模式）

- **一字一词概括**：medieval 就是"中间时代"——历史上指中世纪，口语中指"老掉牙的"
- **延伸中心**：从"历史时期"延伸到"形容落后过时"

> [!check]- 链路验证
> - [x] **可逆性**：从"老式的"能反向推导到"中间时代的"——说某事物 medieval 就是说它像中世纪一样落后
> - [x] **可统筹**：一句话——"medieval 就是中间时代的，引申为落后的"
> - [x] **有支撑**：medieval period, medieval and modern 均有真题支撑

## 词性转换

> [!note]- 词性-词义链接 · L1
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | adj. | 中世纪的；老式的 | 核心词性，med(中间) + ev(时代) + -ial(形容词) |

> [!abstract]- 词尾定性速查
> `-ent`人 / `-ate`使动 / `+ed`持有状态 / `-tion`动作结果 / `-ive`倾向 / `-ous`充满 / `-able`可被 / `-ful`充满 / `-less`没有
>
> 完整规则见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式 · L1
>
> **叙事化词源**（优先）：
> med（中间）+ ev（时代）→ 中间的时代 → 中世纪。古希腊罗马是"古代"，文艺复兴之后是"现代"，中间那约一千年就是 medieval（中世纪）。说一个人的观念 medieval，就是说他"还活在中世纪"——太老式了。
>
> **词拆分记忆**（辅助）：
> med（联想"媒体"）+ evil（邪恶）→ 媒体报道中世纪的邪恶 → 中世纪的

---

## 同义词辨析

> [!note]- 同义词网络 · L2
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[ancient]] | 古代的 | 强调久远的过去 | 文明、历史 | 中性/正面 |
> | [[antique]] | 古老的，古董的 | 强调古老且有价值的 | 家具、艺术品 | 正面 |
> | [[archaic]] | 古老的，过时的 | 强调不再使用的 | 语言、制度 | 中性/消极 |
> | **medieval** | 中世纪的；老式的 | 强调"中间时代"或"落后于现代" | 时期、观念、方法 | 中性/可消极 |
>
> **"增"标记**：medieval **(增-特指欧洲中世纪历史时期)** vs archaic（基准词：仅指过时）

## 反义词

> [!note]- 反义词 · L2
> - [[modern]]：现代的（medieval 的反义：中世纪↔现代）
> - [[contemporary]]：当代的（medieval 的反义：古代↔当代）

## 派生词链接

> [!note]- 派生词族 · L2
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[medieval]] | med(中间) + ev(时代) + -ial | 中世纪的 | 核心词 |
> | [[mediaeval]] | medieval 的变体拼写 | 中世纪的 | 词义补充 |
> | [[medium]] | med(中间) + -ium(名词) | 媒介，中间 | 词义补充（同源） |
> | [[mediate]] | med(中间) + -ate(动词) | 调解，斡旋 | 词义补充（同源） |
> | [[intermediate]] | inter(在……之间) + med(中间) | 中间的， intermediat | 词义补充（同源） |
> | [[medal]] | 源自拉丁语 *metallum* | 奖章，奖牌 | 词族互链（Batch B） |
> | [[medical]] | medic(医) + -al(的) | 医学的，医疗的 | 词族互链（Batch B） |
> | [[medicine]] | medic(医) + -ine(名词) | 药，医学 | 词族互链（Batch B） |
> | [[meet]] | 源自古英语 *mētan* | 遇见，满足 | 词族互链（Batch B） |
> | [[melody]] | mel(歌) + -ody(唱) | 旋律，曲调 | 词族互链（Batch B） |
> | [[melt]] | 源自古英语 *meltan* | 融化，熔化 | 词族互链（Batch B） |

---

## 搭配与短语

> [!example]- 搭配 · L3
> - **medieval period/times**：中世纪时期
> - **medieval Europe**：中世纪欧洲
> - **medieval architecture**：中世纪建筑
> - **the Medieval and modern periods**：中世纪与现代

## 真题/语料关联

> [!example]- 语料 · L3
> - "With the Church's teachings and ways of thinking being eclipsed by the Renaissance, the gap between the Medieval and modern periods had been bridged, leading to new and unexplored intellectual territories."（随着教会的教义和思维方式被文艺复兴所掩盖，中世纪与现代之间的鸿沟已被弥合，从而开辟了新的、未被探索的知识领域。——2020年英语一翻译）
> - "Nothing divided the medieval world in Europe more decisively from the Early Modern period than printing with movable type."（没有什么比活字印刷更能够将中世纪的欧洲和近代早期划分开来。——托福/考研常见学术话题）

## 复习记录

> [!note]- 复习追踪
> **上次复习**：2026-04-23 | **复习次数**：0 | **当前等级**：L0
>
> - [ ] L0→L1：当天 + 1天后（主动回忆原型义）
> - [ ] L1→L2：3天后 + 1周后（主动回忆词义链路+词根）
> - [ ] L2→L3：2周后 + 1个月后（主动回忆辨析+语料）
> - [ ] L3→L4：2个月后（写作/翻译中主动产出）

---

## 主动产出

> [!success]- 内化标记 · L4
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>

---

## 网络激活

> [!todo]- 🌐 网络激活选项
> 回复编号（如 `12` 或 `全选` 或 `跳过`），AI 将勾选对应项并写回。
> `/回顾` 时仅纳入已勾选的网络进行 Layer 2 构建。
>
> - [ ] 词根笔记：[[med-medius-]]（中间，拉丁语 *medius*）
> - [ ] 同义辨析：medieval/ancient/antique/archaic
> - [ ] 反义词群：modern/contemporary
> - [ ] 形近词群：medieval/medium/mediate/intermediate
