---
title: "paradox"
tags:
  - 学习/英语/词汇
  - 语义场/理性世界
aliases: []
date: 2026-04-23
word_freq: 基础词
semantic_field: 理性世界
prototype: 与常理相反的意见（看似矛盾实则可能为真的论点）
extension_dim: [抽象路径]
phonetic: "/ˈpærədɒks/"
pos: n.
word_root: para
network_activation: [词根, 同义辨析, 反义词群, 派生词族]
last_review: 2026-04-23
review_count: 0
---

# paradox

> [!info] 基础信息
> **音标** /ˈpærədɒks/ | **词级** 基础词 | **语义场** [[理性世界]]

## 核心释义

**n.** ①==**悖论，矛盾的说法**== `a paradox of` `resolve a paradox` [正式]；②==**似非而是的论点**==（看似矛盾实则正确）`it is a paradox that` `a curious paradox` [正式]；③自相矛盾的人或事物；

> [!tip] 原型义
> **原型义**：与常理相反的意见（看似矛盾实则可能为真的论点）
> **延伸维度**：抽象路径
> **隐喻类型**：无显著隐喻，字面义即核心义

## 词根词缀

源自希腊语 *paradoxon*，由 para-（与……相反，希腊语 *para*）+ doxa（意见、观点，来自 *dokein*，认为、似乎）构成。字面义是"与普遍意见相反的观点"。
**叙事**：para-（与……相反，参见 [[par]]）+ dox（意见/观点）→ **与常理相反的意见** → 悖论。表面上看自相矛盾，但深入思考后可能发现其中蕴含真理——这就是 paradox。"欲速则不达"就是一个经典的 paradox：越想快反而越慢。千禧一代"穿着最不正式，却最注重个人品牌"也是一个 paradox。

---

## 词义链路

> [!abstract]- 词义链路法
> 以"**与常理相反的意见**"为统筹中心，沿抽象路径向外扩展：
>
> 1. **与常理相反的意见（逻辑层面）**
>    - → **悖论，矛盾的说法**：看似自相矛盾的论点
>      - 激活条件：a logical paradox, resolve a paradox
>    - → **似非而是的论点**：看似矛盾实则正确
>      - 激活条件："More haste, less speed" is a well-known paradox
>    - → **自相矛盾的人或事物**：具有矛盾特征的人或事
>      - 激活条件：He was a paradox — a loner who loved to chat

### 统筹（选择适用的模式）

- **一字一词概括**：paradox 就是"悖论"——看似矛盾实则可能为真
- **延伸中心**：从"与常理相反"延伸到"矛盾中蕴含真理"

> [!check]- 链路验证
> - [x] **可逆性**：从"似非而是"能反向推导到"与常理相反"——正因为与常理相反，所以"看似非而是"
> - [x] **可统筹**：一句话——"paradox 就是与常理相反却可能为真的观点"
> - [x] **有支撑**：a well-known paradox, a curious paradox 均有语料支撑

## 词性转换

> [!note]- 词性-词义链接
>
> | 词性 | 释义 | 转换路径 |
> |------|------|---------|
> | n. | 悖论；似非而是的论点 | 核心词性（仅名词） |

> 词尾定性速查见 [[词汇学习哲学#完整词尾分析规则]]

## 记忆锚点

> [!quote]- 记忆方式
>
> **叙事化词源**（优先）：
> paradox 来自希腊语 paradoxon——para（与……相反）+ dox（意见）。字面义就是"与普遍意见相反的观点"。比如"欲速则不达"——越想快反而越慢，这就是一个 paradox。再比如"如此富有的国家却有这么多穷人"——这又是一个 paradox。paradox 不是"错误"，而是"看似矛盾实则可能为真"的深刻论点。
>
> **词拆分记忆**（辅助）：
> para-（与……相反，参见 [[par]]）+ dox（意见/观点）→ 与常理相反的意见 → 悖论

---

## 同义词辨析

> [!note]- 同义词网络
>
> | 词 | 核心语义差异 | 方式特点 | 常见对象 | 情感色彩 |
> |------|------------|---------|---------|---------|
> | [[contradiction]] | 矛盾 | 强调两个事物之间的对立冲突 | 说法、行为 | 负面/冲突 |
> | [[irony]] | 反讽，讽刺 | 强调结果与预期的相反 | 情境、言语 | 讽刺/幽默 |
> | **paradox** | 悖论；似非而是 | 强调看似矛盾实则可能为真 | 论点、现象 | 智性/深刻 |
>
> **"增"标记**：paradox **(增-智性深度和哲学思辨性)** vs contradiction（基准词：一般矛盾）

## 反义词

> [!note]- 反义词
> - [[axiom]]：公理（paradox 的反义：需要论证的悖论↔不证自明的公理）
> - [[truism]]：不言自明的道理（paradox 的反义：似非而是↔不言自明）

## 派生词链接

> [!note]- 派生词族
>
> | 派生词 | 构词分析 | 释义 | 链接关系 |
> |--------|---------|------|---------|
> | [[paradoxical]] | paradox + -ical | 悖论的；矛盾的 | 派生形容词 |
> | [[paradoxically]] | paradoxical + -ly | 悖论地；矛盾地 | 派生副词 |
> | [[orthodox]] | ortho-（正确）+ dox（意见） | 正统的 | 词根反义（dox-意见） |
> | [[dogma]] | dokein（认为）的派生 | 教条，信条 | 词根同源（dox-意见） |

---

## 搭配与短语

> [!example]- 搭配
> - **a paradox of**：……的悖论
> - **resolve a paradox**：解决悖论
> - **a curious/strange paradox**：奇怪的悖论
> - **present a paradox**：呈现一个悖论

## 真题/语料关联

> [!example]- 语料
> - Millennials face the paradox of being the least formal generation yet the most conscious of style and personal branding. `[真题]`
>   - 中译：【需人工审核】paradox 的例句。
>   - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/paradox
  - 中译：千禧一代面临着这样一个悖论：他们是最不正式的一代人，却最注重风格和个人品牌。；
  - 来源：Oxford | https://www.oxfordlearnersdictionaries.com/definition/english/paradox；
> - The great paradox of our time is that many of us are busy and bored at the same time. `[例]`
  - 中译：我们这个时代最大的悖论是，我们中的许多人同时既忙碌又无聊。；
  - 来源：Cambridge | https://dictionary.cambridge.org/dictionary/english/paradox；
> - It is a paradox that such a rich country should have so many poor people living in it. `[例]`
  - 中译：如此富有的国家却生活着如此多的穷人，这真是一个悖论。；
  - 来源：Merriam-Webster | https://www.merriam-webster.com/dictionary/paradox；

---

## 主动产出

> [!success]- 主动产出
> **写作用例**：<自己在写作/翻译中使用该词的例句>
> **翻译实践**：<翻译练习中的使用记录>
