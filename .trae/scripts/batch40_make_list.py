...
# -*- coding: utf-8 -*-
"""
生成第40批（L0_超纲词 第10批：901–1000）清单：
- 目录：Wiki/L0_超纲词
- 排序：Python sorted(按文件名)
- 切片：files[900:1000]
输出：
- /data/user/work/batch40_files.txt（运行时清单）
- .trae/batches/batch40_files.txt（清单落库，范围基准）
"""
...
OUT_WORK = Path("/data/user/work/batch40_files.txt")
OUT_REPO = REPO / ".trae" / "batches" / "batch40_files.txt"


def main() -> None:
    files = sorted([p for p in DIR.glob("*.md") if p.is_file()], key=lambda p: p.name)
    batch = files[900:1000]
    if len(batch) != 100:
        raise SystemExit(f"Expected 100 files, got {len(batch)} (total={len(files)})")
    if batch[0].name != "subconscious.md" or batch[-1].name != "unprecedented.md":
        raise SystemExit(
