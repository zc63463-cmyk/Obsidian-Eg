...
# -*- coding: utf-8 -*-
"""
第40批（L0_超纲词 第10批：901–1000）联网补齐语料：
- 输入：/data/user/work/batch40_needs_online.tsv
- 输出：
  - /data/user/work/batch40_static_candidates.json
  - /data/user/work/batch40_browser_queue.json
...
from batch40_validate import keep_if_contains_word, looks_like_full_sentence, strip_tag


REPO = Path("/workspace/Obsidian-Eg")
NEEDS_ONLINE = Path("/data/user/work/batch40_needs_online.tsv")
APPLY_STATS = Path("/data/user/work/batch40_apply_stats.json")
REPORT_PATH = REPO / ".trae" / "reports" / "第40批次超纲词901-1000_6.5-6.6_处理报告.md"
BATCH_LIST = REPO / ".trae" / "batches" / "batch40_files.txt"
OUT_CANDIDATES = Path("/data/user/work/batch40_static_candidates.json")
OUT_BROWSER_QUEUE = Path("/data/user/work/batch40_browser_queue.json")
EXAM_CACHE = Path("/data/user/work/batch40_exam_cache.json")
