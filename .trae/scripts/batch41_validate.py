...
# -*- coding: utf-8 -*-
"""
第41批（L0_超纲词 第11批：1001–1050）校验：
- 仅检查规则，不改文件
- 接受 `[例]` / `[真题]` / `[考研-...]`，并兼容已存在的 COCA/BNC 标签
- 拒绝选择题题干、填空残片、列表式伪完整句
输出：
- /data/user/work/batch41_validate_report.json
- /data/user/work/batch41_validate_bad.txt
"""
...
REPORT = Path("/data/user/work/batch41_validate_report.json")
BAD_TXT = Path("/data/user/work/batch41_validate_bad.txt")
