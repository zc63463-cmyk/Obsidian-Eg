...
# -*- coding: utf-8 -*-
"""
第41批（L0_超纲词 第11批：1001–1050）联网补齐语料：
- 输入：/data/user/work/batch41_needs_online.tsv
- 输出：
  - /data/user/work/batch41_static_candidates.json
  - /data/user/work/batch41_browser_queue.json
...
NEEDS_ONLINE = Path("/data/user/work/batch41_needs_online.tsv")
APPLY_STATS = Path("/data/user/work/batch41_apply_stats.json")
REPORT_PATH = REPO / ".trae" / "reports" / "第41批次超纲词1001-1050_6.5-6.6_处理报告.md"
BATCH_LIST = REPO / ".trae" / "batches" / "batch41_files.txt"
OUT_CANDIDATES = Path("/data/user/work/batch41_static_candidates.json")
OUT_BROWSER_QUEUE = Path("/data/user/work/batch41_browser_queue.json")
EXAM_CACHE = Path("/data/user/work/batch41_exam_cache.json")
...
def write_report(
    ...
    lines: List[str] = []
    lines.append("# 第41批次超纲词1001-1050 6.5/6.6 处理报告")
    lines.append("")
    lines.append("## 批次范围")
    lines.append("- 目录：`Wiki/L0_超纲词/`")
    lines.append("- 排序口径：Python `sorted()` 按文件名")
    lines.append("- 切片：`files[1000:1050]`（共100个）")
    if first and last:
