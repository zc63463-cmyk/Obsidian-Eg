...
# -*- coding: utf-8 -*-
"""
第41批（L0_超纲词 第11批：1001–1050）离线 apply：
- 仅修改：
...
输出：
- /data/user/work/batch41_needs_online.tsv
- /data/user/work/batch41_apply_stats.json
"""
...
STATS_JSON = Path("/data/user/work/batch41_apply_stats.json")

DIGITS = "①②③④⑤⑥⑦⑧⑨⑩"
ANY_TAG_RE = re.compile(r"`\[[^\]]+\]`\s*$")
WORD_PATTERN_OVERRIDES = {
    "tamper": "V with N",
    "succumb": "V to N",
    "theorize": "V about/on N",
    "supposing": "that-clause",
    "transcribe": "V N into N",
    "undercut": "V N",
    "subsidize": "V N",
    "unleash": "V N",
    "traverse": "V N",
...
def pick_pattern(word: str, pos: str, meaning_zh: str) -> Optional[str]:
    override = WORD_PATTERN_OVERRIDES.get(word.lower().strip())
    if override:
        return override
    pos = pos.strip().lower()
    pos = re.sub(r"\s+", "", pos)
...
def apply_65(word: str, core_text: str) -> Tuple[str, bool]:
    ...
    for line in core_text2.splitlines(keepends=True):
        ...
        def add_pat(mm: re.Match) -> str:
            ...
            digit = mm.group(1)
            meaning = mm.group(2)
            pat = pick_pattern(word, pos, meaning)
            if not pat:
                return mm.group(0)
            changed = True
...
def main() -> None:
    ...
    for rel in rels:
        ...
        if core_span:
            ...
            new_core, changed65 = apply_65(word, core)
            if changed65:
